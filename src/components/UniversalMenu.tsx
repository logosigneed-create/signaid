    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black flex flex-col items-center justify-center animate-fade-in">
            ...
            <nav className="flex flex-col gap-4 md:gap-8 text-center max-h-[80vh] overflow-y-auto">
                ...
                <button
                    onClick={() => { onClose(); onNavigate('blog'); }}
                    className={getItemClass('blog')}
                >
                    Blog
                </button>
                <button
                ...
                <a
                    href="/"
                    className="text-lg md:text-xl font-medium text-gray-400 hover:text-white mt-4 transition-colors uppercase tracking-widest border-b border-transparent hover:border-white pb-1"
                >
                    Instructions
                </a>

                {/* Primary Mobile CTA */}
                <div className="mt-12 px-8 w-full max-w-xs">
                    <button 
                        onClick={() => { onClose(); onNavigate(user ? 'profile' : 'profile'); }}
                        className="w-full py-4 bg-orange-600 text-white font-black rounded-2xl uppercase tracking-widest text-sm shadow-xl shadow-orange-500/20 active:scale-95 transition-all"
                    >
                        {user ? 'Mon Profil' : 'Connexion / Inscription'}
                    </button>
                </div>
            </nav>
        </div>
