...
export
            ...
            reader.onload = async (event) => {
                if (event.target?.result) {
                    ...
                    if (aiModalOpen || isAiViewVisible) {
                        ...
                        console.log(`[Standard Upload] Smart placement and setup crop.`);

                        let targetSlot = activeEl || 'logo';
                        const hasL1 = isBack ? item.originalLogoUrlBack || item.predefinedLogoUrlBack : item.originalLogoUrlFront || item.predefinedLogoUrlFront;
                        const hasL2 = isBack ? item.logoBack2?.originalUrl || item.logoBack2?.predefinedUrl : item.logoFront2?.originalUrl || item.logoFront2?.predefinedUrl;
                        const hasL3 = isBack ? item.logoBack3?.originalUrl || item.logoBack3?.predefinedUrl : item.logoFront3?.originalUrl || item.logoFront3?.predefinedUrl;

                        if (targetSlot === 'logo' && hasL1) {
                            if (!hasL2) targetSlot = 'logo2';
                            else if (!hasL3) targetSlot = 'logo3';
                        } else if (targetSlot === 'logo2' && hasL2) {
                            if (!hasL1) targetSlot = 'logo';
                            else if (!hasL3) targetSlot = 'logo3';
                        } else if (targetSlot === 'logo3' && hasL3) {
                            if (!hasL1) targetSlot = 'logo';
                            else if (!hasL2) targetSlot = 'logo2';
                        }
                        ...
                        if (targetSlot === 'logo2') {
                            const key = isBack ? 'logoBack2' : 'logoFront2';
                            updates[key] = { ...item[key], originalUrl: finalImage, processedUrl_original: finalImage, isPredefined: false, predefinedUrl: null, processedUrl: null, backgroundRemoved: false, inverted: false, activeColor: 'original', processedUrl_noBackground: null }; 
                            if (!item[key]?.position) { updates[key].position = defaults; updates[key].size = defaults.scale; }
                        } else if (targetSlot === 'logo3') {
                            const key = isBack ? 'logoBack3' : 'logoFront3';
                            updates[key] = { ...item[key], originalUrl: finalImage, processedUrl_original: finalImage, isPredefined: false, predefinedUrl: null, processedUrl: null, backgroundRemoved: false, inverted: false, activeColor: 'original', processedUrl_noBackground: null }; 
                            if (!item[key]?.position) { updates[key].position = defaults; updates[key].size = defaults.scale; }
                        } else {
                            if (isBack) {
                                updates = { originalLogoUrlBack: finalImage, processedLogoUrlBack_original: finalImage, isPredefinedLogoBack: false, predefinedLogoUrlBack: null, processedLogoUrlBack: null, backgroundRemovedBack: false, logoInvertedBack: false, activeLogoColorBack: 'original', processedLogoUrlBack_white: null, processedLogoUrlBack_black: null, processedLogoUrlBack_noBackground: null };
                            } else {
                                updates = { originalLogoUrlFront: finalImage, processedLogoUrlFront_original: finalImage, isPredefinedLogoFront: false, predefinedLogoUrlFront: null, processedLogoUrlFront: null, backgroundRemovedFront: false, logoInvertedFront: false, activeLogoColorFront: 'original', processedLogoUrlFront_white: null, processedLogoUrlFront_black: null, processedLogoUrlFront_noBackground: null };
                            }
                        }

                        updateItem(updates);
                        setUploadedLogoPreview(finalImage);
                        setActiveEl(targetSlot);
    ...
    useEffect(() => {
        if (aiResult) {
            try {
                ...
                console.warn("Could not save AI result to local storage (quota exceeded)", e);
            }
        }
    }, [aiResult]);
    ...
    */
    ...
    const handleCropSave = async () => {
        if (croppingImage && cropImgRef.current) {
            try {
                ...
                const targetSlot = activeEl || 'logo';
                let updates: any = {};

                if (targetSlot === 'logo2') {
                    const key = targetIsBack ? 'logoBack2' : 'logoFront2';
                    updates[key] = { ...item[key], processedUrl_original: finalImage, originalUrl: finalImage, processedUrl: null, backgroundRemoved: false, inverted: false, activeColor: 'original', processedUrl_noBackground: null };
                } else if (targetSlot === 'logo3') {
                    const key = targetIsBack ? 'logoBack3' : 'logoFront3';
                    updates[key] = { ...item[key], processedUrl_original: finalImage, originalUrl: finalImage, processedUrl: null, backgroundRemoved: false, inverted: false, activeColor: 'original', processedUrl_noBackground: null };
                } else {
                    updates = targetIsBack ? {
                        processedLogoUrlBack_original: finalImage, originalLogoUrlBack: finalImage, backgroundRemovedBack: false, logoInvertedBack: false, processedLogoUrlBack: null, activeLogoColorBack: 'original', processedLogoUrlBack_white: null, processedLogoUrlBack_black: null, processedLogoUrlBack_noBackground: null
                    } : {
                        processedLogoUrlFront_original: finalImage, originalLogoUrlFront: finalImage, backgroundRemovedFront: false, logoInvertedFront: false, processedLogoUrlFront: null, activeLogoColorFront: 'original', processedLogoUrlFront_white: null, processedLogoUrlFront_black: null, processedLogoUrlFront_noBackground: null
                    };
                }
    ...
    const processLogoUpdate = (dataUrl: string) => {
        ...
        if (targetSlot === 'logo2') {
            const key = isBack ? 'logoBack2' : 'logoFront2';
            updates[key] = { originalUrl: dataUrl, processedUrl_original: dataUrl, isPredefined: false, predefinedUrl: null, position: defaults, size: defaults.scale, backgroundRemoved: false, inverted: false, activeColor: 'original', processedUrl: null, processedUrl_noBackground: null };
        } else if (targetSlot === 'logo3') {
            const key = isBack ? 'logoBack3' : 'logoFront3';
            updates[key] = { originalUrl: dataUrl, processedUrl_original: dataUrl, isPredefined: false, predefinedUrl: null, position: defaults, size: defaults.scale, backgroundRemoved: false, inverted: false, activeColor: 'original', processedUrl: null, processedUrl_noBackground: null };
        } else {
            if (isBack) {
                updates = { originalLogoUrlBack: dataUrl, processedLogoUrlBack_original: dataUrl, isPredefinedLogoBack: false, predefinedLogoUrlBack: null, logoPositionXBack: defaults.x, logoPositionYBack: defaults.y, logoSizeBack: defaults.scale, backgroundRemovedBack: false, logoInvertedBack: false, activeLogoColorBack: 'original', processedLogoUrlBack: null, processedLogoUrlBack_noBackground: null };
            } else {
                updates = { originalLogoUrlFront: dataUrl, processedLogoUrlFront_original: dataUrl, isPredefinedLogoFront: false, predefinedLogoUrlFront: null, logoPositionXFront: defaults.x, logoPositionYFront: defaults.y, logoSizeFront: defaults.scale, backgroundRemovedFront: false, logoInvertedFront: false, activeLogoColorFront: 'original', processedLogoUrlFront: null, processedLogoUrlFront_noBackground: null };
            }
        }
    ...
    const handlePredefinedLogoSelect = (url: string) => {
        ...
        if (targetSlot === 'logo2') {
            const key = isBack ? 'logoBack2' : 'logoFront2';
            updateItemUndoable({ [key]: { originalUrl: null, processedUrl_original: proxiedUrl, processedUrl: null, processedUrl_noBackground: null, isPredefined: true, predefinedUrl: proxiedUrl, position: defaults, size: defaults.scale, activeColor: 'original', backgroundRemoved: false } as any });
        } else if (targetSlot === 'logo3') {
            const key = isBack ? 'logoBack3' : 'logoFront3';
            updateItemUndoable({ [key]: { originalUrl: null, processedUrl_original: proxiedUrl, processedUrl: null, processedUrl_noBackground: null, isPredefined: true, predefinedUrl: proxiedUrl, position: defaults, size: defaults.scale, activeColor: 'original', backgroundRemoved: false } as any });
        } else {
            updateItemUndoable(isBack
                ? { predefinedLogoUrlBack: proxiedUrl, originalLogoUrlBack: null, processedLogoUrlBack: null, processedLogoUrlBack_original: proxiedUrl, processedLogoUrlBack_noBackground: null, isPredefinedLogoBack: true, logoPositionXBack: defaults.x, logoPositionYBack: defaults.y, logoSizeBack: defaults.scale, activeLogoColorBack: 'original', backgroundRemovedBack: false }
                : { predefinedLogoUrlFront: proxiedUrl, originalLogoUrlFront: null, processedLogoUrlFront: null, processedLogoUrlFront_original: proxiedUrl, processedLogoUrlFront_noBackground: null, isPredefinedLogoFront: true, logoPositionXFront: defaults.x, logoPositionYFront: defaults.y, logoSizeFront: defaults.scale, activeLogoColorFront: 'original', backgroundRemovedFront: false }
            );
        }
        setActiveEl(targetSlot);
    };
                                ...
                                >
                                    ...
                                    {item.productType === 'catalogue' ? (
                                        ...
                                        <>
                                            ...
                                            {/* Slot 2 Logo */}
                                            {(isBack ? item.logoBack2?.originalUrl || item.logoBack2?.predefinedUrl : item.logoFront2?.originalUrl || item.logoFront2?.predefinedUrl) && (
                                                <DraggableElement
                                            ...
                                            {/* Slot 3 Logo */}
                                            {(isBack ? item.logoBack3?.originalUrl || item.logoBack3?.predefinedUrl : item.logoFront3?.originalUrl || item.logoFront3?.predefinedUrl) && (
                                                <DraggableElement
                    ...
                    activePanel === 'import' && createPortal(
                        <div data-layout-id="desktop-panel-import" className="fixed inset-0 z-[9999] flex items-end lg:items-start justify-center lg:justify-end bg-transparent lg:pt-16 p-0 lg:p-4 pointer-events-none" style={{ display: 'flex', opacity: 1, visibility: 'visible', top: 0 }}>
                            <div
                                ...
                                style={window.innerWidth >= 1024 ? { transform: 'translate(' + panelOffsets.import.x + 'px, ' + panelOffsets.import.y + 'px)', marginTop: '0' } : { transform: 'none', marginTop: '0' }}
                            ...
                            >
                                ...
                                <div className="flex justify-between items-center mb-2 no-drag">
                                ...
                                {/* ONGLETS MULTI-LOGOS */}
                                <div className="flex bg-gray-100 p-1 rounded-lg gap-1 no-drag">
                                    <button
                                        onClick={(e) => { e.preventDefault(); setActiveEl('logo'); }}
                                        className={"flex-1 py-1.5 text-xs font-bold rounded-md transition-all " + ((activeEl !== 'logo2' && activeEl !== 'logo3') ? "bg-white shadow text-orange-600" : "text-gray-500 hover:bg-gray-200")}
                                    >
                                        Logo 1
                                    </button>
                                    <button
                                        onClick={(e) => { e.preventDefault(); setActiveEl('logo2'); }}
                                        className={"flex-1 py-1.5 text-xs font-bold rounded-md transition-all " + (activeEl === 'logo2' ? "bg-white shadow text-orange-600" : "text-gray-500 hover:bg-gray-200")}
                                    >
                                        Logo 2
                                    </button>
                                    <button
                                        onClick={(e) => { e.preventDefault(); setActiveEl('logo3'); }}
                                        className={"flex-1 py-1.5 text-xs font-bold rounded-md transition-all " + (activeEl === 'logo3' ? "bg-white shadow text-orange-600" : "text-gray-500 hover:bg-gray-200")}
                                    >
                                        Logo 3
                                    </button>
                                </div>
                                ...
                                {(() => {
                                    // LOGIQUE MULTI-LOGOS INTERNE
                                    const getActiveLogoData = () => {
                                        if (activeEl === 'logo2') {
                                            const data = isBack ? item.logoBack2 : item.logoFront2;
                                            return { originalUrl: data?.originalUrl || null, processedUrl_original: data?.processedUrl_original || null, processedUrl_noBackground: data?.processedUrl_noBackground || null, predefinedUrl: data?.predefinedUrl || null, backgroundRemoved: data?.backgroundRemoved || false, activeColor: data?.activeColor || 'original' };
                                        } else if (activeEl === 'logo3') {
                                            const data = isBack ? item.logoBack3 : item.logoFront3;
                                            return { originalUrl: data?.originalUrl || null, processedUrl_original: data?.processedUrl_original || null, processedUrl_noBackground: data?.processedUrl_noBackground || null, predefinedUrl: data?.predefinedUrl || null, backgroundRemoved: data?.backgroundRemoved || false, activeColor: data?.activeColor || 'original' };
                                        } else {
                                            return { originalUrl: isBack ? item.originalLogoUrlBack : item.originalLogoUrlFront, processedUrl_original: isBack ? item.processedLogoUrlBack_original : item.processedLogoUrlFront_original, processedUrl_noBackground: isBack ? item.processedLogoUrlBack_noBackground : item.processedLogoUrlFront_noBackground, predefinedUrl: isBack ? item.predefinedLogoUrlBack : item.predefinedLogoUrlFront, backgroundRemoved: isBack ? item.backgroundRemovedBack : item.backgroundRemovedFront, activeColor: isBack ? item.activeLogoColorBack : item.activeLogoColorFront };
                                        }
                                    };

                                    // Upload intelligent
                                    const triggerSmartUpload = (e: any) => {
                                        if (e) e.stopPropagation();

                                        let target = activeEl || 'logo';
                                        const hasL1 = isBack ? item.originalLogoUrlBack || item.predefinedLogoUrlBack : item.originalLogoUrlFront || item.predefinedLogoUrlFront;
                                        const hasL2 = isBack ? item.logoBack2?.originalUrl || item.logoBack2?.predefinedUrl : item.logoFront2?.originalUrl || item.logoFront2?.predefinedUrl;
                                        const hasL3 = isBack ? item.logoBack3?.originalUrl || item.logoBack3?.predefinedUrl : item.logoFront3?.originalUrl || item.logoFront3?.predefinedUrl;

                                        if (target === 'logo' && hasL1) {
                                            if (!hasL2) target = 'logo2';
                                            else if (!hasL3) target = 'logo3';
                                        } else if (target === 'logo2' && hasL2) {
                                            if (!hasL1) target = 'logo';
                                            else if (!hasL3) target = 'logo3';
                                        } else if (target === 'logo3' && hasL3) {
                                            if (!hasL1) target = 'logo';
                                            else if (!hasL2) target = 'logo2';
                                        }

                                        setActiveEl(target);

                                        // On utilise le input natif de CustomizerView, processLogoUpdate fera le reste!
                                        document.getElementById('hidden-file-input-desktop')?.click();
                                    };

                                    const logoData = getActiveLogoData();
                                    const hasContent = !!(logoData.originalUrl || logoData.predefinedUrl || logoData.processedUrl_original);
                                        ...
                                        return (
                                            <>
                                                <div className="flex items-center justify-between gap-4 mb-2 pr-10">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <button onClick={undo} disabled={undoStack.length === 0} className={`w-10 h-10 rounded-full flex items-center justify-center border transition-all ${undoStack.length === 0 ? 'bg-gray-50 text-gray-300 border-gray-100 cursor-not-allowed' : 'bg-white text-gray-600 border-gray-200 hover:bg-orange-50 hover:text-orange-600 hover:border-orange-200 shadow-sm active:scale-90'}`} title="Annuler">
                                                            <i className="fa-solid fa-rotate-left"></i>
                                                        </button>
                                                        <button onClick={redo} disabled={redoStack.length === 0} className={`w-10 h-10 rounded-full flex items-center justify-center border transition-all ${redoStack.length === 0 ? 'bg-gray-50 text-gray-300 border-gray-100 cursor-not-allowed' : 'bg-white text-gray-600 border-gray-200 hover:bg-orange-50 hover:text-orange-600 hover:border-orange-200 shadow-sm active:scale-90'}`} title="Répéter">
                                                ...
                                                <div className="flex items-center justify-between gap-2 overflow-x-auto pb-2 no-drag">
                                                    <button
                                                        onClick={async () => {
                                                            ...
                                                            try {
                                                                ...
                                                                if (activeEl === 'logo2') {
                                                                    const targetKey = isBack ? 'logoBack2' : 'logoFront2';
                                                                    // @ts-ignore
                                                                    updateItem({ [targetKey]: { ...item[targetKey], backgroundRemoved: true, processedUrl: processed, processedUrl_original: processed, processedUrl_noBackground: processed } });
                                                                } else if (activeEl === 'logo3') {
                                                                    const targetKey = isBack ? 'logoBack3' : 'logoFront3';
                                                                    // @ts-ignore
                                                                    updateItem({ [targetKey]: { ...item[targetKey], backgroundRemoved: true, processedUrl: processed, processedUrl_original: processed, processedUrl_noBackground: processed } });
                                                                } else {
                                                                    updateItem(isBack
                                                    ...
                                                    <button
                                                        onClick={() => {
                                                            ...
                                                            if (activeEl === 'logo2') {
                                                                if (sourceIsBack) updateItem({ logoFront2: item.logoBack2 });
                                                                else updateItem({ logoBack2: item.logoFront2 });
                                                            } else if (activeEl === 'logo3') {
                                                                if (sourceIsBack) updateItem({ logoFront3: item.logoBack3 });
                                                                else updateItem({ logoBack3: item.logoFront3 });
                                                            } else {
                                                                if (sourceIsBack) {
                                                                    updateItem({
                                                                        originalLogoUrlFront: item.originalLogoUrlBack, predefinedLogoUrlFront: item.predefinedLogoUrlBack,
                                                                        processedLogoUrlFront: item.processedLogoUrlBack, processedLogoUrlFront_original: item.processedLogoUrlBack_original,
                                                                        processedLogoUrlFront_noBackground: item.processedLogoUrlBack_noBackground, backgroundRemovedFront: item.backgroundRemovedBack,
                                                                        activeLogoColorFront: item.activeLogoColorBack, logoInvertedFront: item.logoInvertedBack,
                                                                        logoSizeFront: item.logoSizeBack, logoPositionXFront: item.logoPositionXBack,
                                                                        logoPositionYFront: item.logoPositionYBack, logoAspectRatioFront: item.logoAspectRatioBack
                                                                    });
                                                                } else {
                                                                    updateItem({
                                                                        originalLogoUrlBack: item.originalLogoUrlFront, predefinedLogoUrlBack: item.predefinedLogoUrlFront,
                                                                        processedLogoUrlBack: item.processedLogoUrlFront, processedLogoUrlBack_original: item.processedLogoUrlFront_original,
                                                                        processedLogoUrlBack_noBackground: item.processedLogoUrlFront_noBackground, backgroundRemovedBack: item.backgroundRemovedFront,
                                                                        activeLogoColorBack: item.activeLogoColorFront, logoInvertedBack: item.logoInvertedFront,
                                                                        logoSizeBack: item.logoSizeFront, logoPositionXBack: item.logoPositionXFront,
                                                                        logoPositionYBack: item.logoPositionYFront, logoAspectRatioBack: item.logoAspectRatioFront
                                                                    });
                                                                }
                                                            }
                                                        }}
                                                    ...
                                                    <button
                                                        onClick={() => {
                                                            if (activeEl === 'logo2') {
                                                                updateItem({ [isBack ? 'logoBack2' : 'logoFront2']: undefined });
                                                            } else if (activeEl === 'logo3') {
                                                                updateItem({ [isBack ? 'logoBack3' : 'logoFront3']: undefined });
                                                            } else {
                                                                updateItem(isBack
                                                                    ? { originalLogoUrlBack: null, processedLogoUrlBack_original: null, isPredefinedLogoBack: false, predefinedLogoUrlBack: null, processedLogoUrlBack: null, processedLogoUrlBack_noBackground: null, processedLogoUrlBack_white: null, processedLogoUrlBack_black: null }
                                                                    : { originalLogoUrlFront: null, processedLogoUrlFront_original: null, isPredefinedLogoFront: false, predefinedLogoUrlFront: null, processedLogoUrlFront: null, processedLogoUrlFront_noBackground: null, processedLogoUrlFront_white: null, processedLogoUrlFront_black: null }
                                                                );
                                                            }
                                                            setActiveEl(null);
                                                            if (typeof setUploadedLogoPreview === 'function') setUploadedLogoPreview(null);
                                                        }}
                                                ...
                                                <div className="flex flex-col gap-2 mt-4 no-drag">
                                                    ...
                                                    <div className="flex flex-wrap gap-2">
                                                        <button
                                                            onClick={async () => {
                                                                const sourceUrl = (logoData.backgroundRemoved && logoData.processedUrl_noBackground) ? logoData.processedUrl_noBackground : (logoData.processedUrl_original || logoData.originalUrl || logoData.predefinedUrl);
                                                                if (!sourceUrl) return;
                                                                let newProcessedUrl = logoData.processedUrl_noBackground || logoData.processedUrl_original;

                                                                if (activeEl === 'logo2') {
                                                                    const targetKey = isBack ? 'logoBack2' : 'logoFront2';
                                                                    // @ts-ignore
                                                                    updateItemUndoable({ [targetKey]: { ...item[targetKey], activeColor: 'original', processedUrl: newProcessedUrl } });
                                                                } else if (activeEl === 'logo3') {
                                                                    const targetKey = isBack ? 'logoBack3' : 'logoFront3';
                                                                    // @ts-ignore
                                                                    updateItemUndoable({ [targetKey]: { ...item[targetKey], activeColor: 'original', processedUrl: newProcessedUrl } });
                                                                    ...
                                                                    const sideKey = isBack ? 'Back' : 'Front';
                                                                    updateItemUndoable({ [`activeLogoColor${sideKey}`]: 'original', [`processedLogoUrl${sideKey}`]: newProcessedUrl });
                                                                }
                                                            }}
                                                        ...
                                                        {['#000000', '#FFFFFF', '#EF4444', '#3B82F6', '#F59E0B', '#10B981', '#8B5CF6', '#EC4899', '#FFD700'].map(c => (
                                                            <button
                                                                ...
                                                                onClick={async () => {
                                                                    ...
                                                                    if (!sourceUrl) return;
                                                                    let newProcessedUrl = null;
                                                                    try { newProcessedUrl = await processLogoColor(sourceUrl, c); } catch (e) { }

                                                                    if (activeEl === 'logo2') {
                                                                        const targetKey = isBack ? 'logoBack2' : 'logoFront2';
                                                                        // @ts-ignore
                                                                        updateItemUndoable({ [targetKey]: { ...item[targetKey], activeColor: c, processedUrl: newProcessedUrl } });
                                                                    } else if (activeEl === 'logo3') {
                                                                        const targetKey = isBack ? 'logoBack3' : 'logoFront3';
                                                                        // @ts-ignore
                                                                        updateItemUndoable({ [targetKey]: { ...item[targetKey], activeColor: c, processedUrl: newProcessedUrl } });
                                                                        ...
                                                                        const sideKey = isBack ? 'Back' : 'Front';
                                                                        updateItemUndoable({ [`activeLogoColor${sideKey}`]: c, [`processedLogoUrl${sideKey}`]: newProcessedUrl });
                                                                    }
                                                                }}
                                                                className={`flex-shrink-0 w-10 h-10 rounded-xl border-2 transition-all hover:scale-105 shadow-sm ${logoData.activeColor === c ? 'ring-2 ring-orange-500 ring-offset-2 scale-105 border-white' : 'border-gray-100'}`}
