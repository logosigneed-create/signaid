...
export function CustomizerView({ 
    ...
    const [aiResults, setAiResults] = useState<{ front?: string, back?: string } | null>(null);
    ...
    */
        ...
        try {
            ...
            if (onGenerateAi) {
                ...
                try {
                    ...
                    alert("La génération IA a échoué.");
                    setAiStep('input');
                } finally {
    ...
    // Vérifie si au moins un article a été sélectionné dans les tailles
    const totalQtySelected = Object.values(selectedSizes).reduce((acc, qty) => acc + (qty as number), 0);
    const hasAnySize = totalQtySelected > 0;
    ...
    const handleAddToCartAction = async () => {
        ...
        try {
            ...
            if (!hasSizes) {
                ...
                // Toast/Alert to force feedback
                if (showToast) showToast("Veuillez sélectionner une taille !", "error");
    ...
    const mobileTools = [
        ...
        { id: 'image', icon: 'fa-image', label: 'Image', action: handleImportButtonClick },
        {
            ...
            label: aiResult ? 'Voir IA' : 'Essayage',
            action: () => {
                if (aiResult) {
                    setShowAiResultModal(true);
                } else {
    ...
    return (
        <div className="flex flex-col min-h-[100dvh] lg:h-full w-full max-w-[1025px] mx-auto relative lg:pb-24 bg-transparent overflow-hidden lg:overflow-visible justify-start" style={{ touchAction: 'pan-y' }}>
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col gap-0">
                                            ...
                                            >
                                                ...
                                                <div className="text-left">
                                                    <span className="text-sm font-black text-gray-900 block">Essayage intelligent</span>
                                                    <span className="text-[10px] text-gray-400 font-bold">Preview 1:1 sur votre photo</span>
                                                </div>
                                            </button>
                                        </div>
                                    </div>
                        ...
                        <div className="relative group w-full max-w-md">
                            ...
                            <div className="flex gap-3 w-full">
                                ...
                                <button
                                    ...
                                    disabled={isCapturing}
                                    className={`flex-[2] py-5 rounded-2xl font-black uppercase tracking-widest text-sm shadow-xl transition-all flex items-center justify-center gap-3 active:scale-95 ${
                                        hasAnySize 
                                            ? 'bg-green-500 text-white hover:bg-green-400 hover:shadow-green-500/20 hover:scale-[1.02]' 
                                            : 'bg-gray-900 text-white hover:bg-black hover:shadow-gray-900/40'
                                    }`}
                                >
                                    <i className={`fa-solid ${
                                        isCapturing ? 'fa-circle-notch fa-spin' : 
                                        hasAnySize ? 'fa-cart-shopping' : 'fa-check-double'
                                    }`}></i>
                                    <span>
                                        {isCapturing 
                                            ? 'Patientez...' 
            ...
            <div className="tools-portals-container">
                ...
                {/* AI RESULT PREVIEW MODAL - PREMIUM DOUBLE VIEW */}
                {showAiResultModal && (aiResults || aiResult) && createPortal(
                    <div className="fixed inset-0 z-[10001] bg-black/95 backdrop-blur-md flex items-center justify-center animate-fade-in p-4 md:p-8">
                        ...
                        <div className="flex flex-col md:flex-row w-full h-full max-w-7xl mx-auto gap-6 md:gap-12 items-center justify-center overflow-y-auto">
                            {/* IMAGE CONTAINER(S) */}
                            <div className="flex-1 w-full flex flex-col md:flex-row items-center justify-center gap-6 overflow-visible py-10">
                                {aiResults?.front && (
                                    <div className="h-[50vh] md:h-[70vh] aspect-[9/16] relative rounded-3xl overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-white/10 bg-black/20 group animate-slide-up">
                                        <div className="absolute top-4 left-4 z-10 px-3 py-1 bg-black/40 backdrop-blur-md rounded-full border border-white/20">
                                            <span className="text-[10px] font-black text-white uppercase tracking-widest">Face</span>
                                        </div>
                                ...
                                {aiResults?.back && (
                                    <div className="h-[50vh] md:h-[70vh] aspect-[9/16] relative rounded-3xl overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-white/10 bg-black/20 group animate-slide-up" style={{ animationDelay: '0.2s' }}>
                                        <div className="absolute top-4 left-4 z-10 px-3 py-1 bg-black/40 backdrop-blur-md rounded-full border border-white/20">
                                            <span className="text-[10px] font-black text-white uppercase tracking-widest">Dos</span>
                                        </div>
                                ...
                                {!aiResults && aiResult && (
                                    <div className="h-[65vh] md:h-[85vh] aspect-[9/16] relative rounded-3xl overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-white/10 bg-black/20 animate-fade-in">
                            ...
                            {/* ACTIONS PANEL */}
                            <div className="w-full md:w-96 flex-shrink-0 flex flex-col gap-4 bg-white/5 backdrop-blur-2xl p-8 rounded-[40px] border border-white/10 shadow-2xl">
                                <div className="text-center mb-4">
                                    <div className="w-16 h-16 rounded-full bg-green-500/20 flex items-center justify-center mx-auto mb-4 border border-green-500/30">
                                        <i className="fa-solid fa-wand-magic-sparkles text-2xl text-green-400"></i>
                                    </div>
                                    <h3 className="text-3xl font-black text-white tracking-tight uppercase">C'est Prêt !</h3>
                                    <p className="text-white/50 font-bold uppercase tracking-widest text-[10px] mt-2">Votre design personnalisé par l'IA</p>
                                </div>
                                ...
                                <button
                                    ...
                                    className="w-full py-6 bg-gradient-to-r from-green-600 to-green-400 text-white font-black uppercase tracking-widest rounded-2xl shadow-xl shadow-green-900/40 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-4 group"
                                >
                                    <span>Commander ce design</span>
                                    <i className="fa-solid fa-cart-shopping text-xl group-hover:translate-x-1 transition-transform"></i>
                                </button>
                                ...
                                <button
                                    ...
                                    className="w-full py-4 bg-white/10 text-white/80 font-bold rounded-2xl hover:bg-white/20 hover:text-white transition-all flex items-center justify-center gap-3 border border-white/5"
                                >
                                ...
                                <button
                                    ...
                                    onClick={async () => {
                                        if (!user) {
                                            ...
                                            if (btn) {
                                                ...
                                                try {
                                                    await onSaveToProfile(aiResult || aiResults?.front || '', selectedAiPrompt || customStylePrompt, selectedStyleName || activeStyleCategory, item);
                                    ...
                                    className="w-full py-3 bg-white/5 text-white/50 text-[10px] font-bold rounded-xl border border-white/10 hover:bg-white/10 transition-all uppercase tracking-widest"
                                >
                                    {user ? 'Sauvegarder dans mon profil' : 'Se connecter pour sauvegarder'}
                                </button>
                            </div>
                        </div>
                    </div>,
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        <div className="flex gap-2 w-full mt-4">
                                            ...
                                            <button
                                                onClick={handleAddToCartAction} 
                                                disabled={isCapturing}
                                                className={`flex-[2] py-4 text-white font-black uppercase tracking-wider rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 ${
                                                    hasAnySize 
                                                        ? 'bg-green-500 hover:bg-green-400 shadow-green-500/20' // Vert si prêt pour le panier
                                                        : 'bg-orange-600 hover:bg-orange-500 shadow-orange-500/20' // Orange pour Valider
                                                }`}
                                            >
                                                <i className={`fa-solid ${
                                                    isCapturing ? 'fa-circle-notch fa-spin' : 
