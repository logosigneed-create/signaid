...
export
    ...
    // AI Variant Cache: Map 'productType_color' to 'aiImageUrl'
    const [aiVariantCache, setAiVariantCache] = useState<Record<string, string>>(() => {
        try {
            const saved = localStorage.getItem('stylelink_ai_variant_cache');
            return saved ? JSON.parse(saved) : {};
        } catch (e) {
            console.warn("Failed to load aiVariantCache", e);
            return {};
        }
    });
            ...
            reader.onload = async (event) => {
                if (event.target?.result) {
                    ...
                    if (aiModalOpen || isAiViewVisible) {
                        ...
                        // STANDARD UPLOAD MODE — multi-slot support
                        console.log(`[Standard Upload] Slot ${activeLogoSlot}, isBack: ${isBack}`);
                        ...
                        if (activeLogoSlot === 1) {
                        ...
                        } else if (activeLogoSlot === 2) {
                            const slotKey = targetIsBack ? 'logoBack2' : 'logoFront2';
                            const existing = targetIsBack ? item.logoBack2 : item.logoFront2;
                            updates = {
                                [slotKey]: {
                                    ...existing,
                                    ...
                                    position: existing?.position || { x: 62, y: 30 },
                                    size: existing?.size || 80,
                            ...
                            setUploadedLogoPreview(finalImage);
                            setActiveEl('logo2');
                        } else if (activeLogoSlot === 3) {
                            const slotKey = targetIsBack ? 'logoBack3' : 'logoFront3';
                            const existing = targetIsBack ? item.logoBack3 : item.logoFront3;
                            updates = {
                                [slotKey]: {
                                    ...existing,
                                    originalUrl: finalImage,
                                    ...
                                    backgroundRemoved: false,
                                    activeColor: 'original',
                                    position: existing?.position || { x: 38, y: 60 },
                                    size: existing?.size || 80,
                                    aspectRatio: existing?.aspectRatio || 1
                                }
                            };
    ...
    useEffect(() => {
        if (item.color !== lastColorRef.current) {
            const cacheKey = `${item.productType}_${item.color}`;
            const cachedAiImage = aiVariantCache[cacheKey];

            if (setAiResult) setAiResult(cachedAiImage || null);
            updateItem({ aiImageUrl: cachedAiImage || null });
            setIsAiViewVisible(!!cachedAiImage);
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            try {
                ...
                if (aiResult) {
                    // CRITICAL: Quota check (LocalStorage is ~5MB)
                    // If AI result (Base64) is too big, don't save to LS, but keep in memory.
                    // 1500000 chars is ~1.5MB which is safe.
                    if (aiResult.length < 1500000) {
                        localStorage.setItem('stylelink_draft_ai_result', aiResult);
                    } else {
                        console.warn("[Storage] AI Result too large for LocalStorage, skipping persist.");
                ...
                console.error("Draft save quota reached - clearing old AI results", e);
                // Emergency cleanup
                localStorage.removeItem('stylelink_draft_ai_result');
                localStorage.removeItem('stylelink_ai_variant_cache');
            }
    ...
    // PERSIST AI VARIANT CACHE
    useEffect(() => {
        try {
            localStorage.setItem('stylelink_ai_variant_cache', JSON.stringify(aiVariantCache));
        } catch (e) {
            console.warn("Failed to save aiVariantCache", e);
        }
    }, [aiVariantCache]);
    ...
    const [activeLogoSlot, setActiveLogoSlot] = useState<1 | 2 | 3>(1);
    ...
    const [hasShownColorWarning, setHasShownColorWarning] = useState(false);

    const [previewImage, setPreviewImage] = useState<string | null>(null);
    const [processedProductImages, setProcessedProductImages] = useState<Record<string, string>>({});
    ...
    useEffect(() => {
        ...
        const processProductImage = async () => {
            try {
                ...
                if (trimmed && trimmed.blobUrl) {
                    setProcessedProductImages(prev => ({
                        ...prev,
                        [cacheKey]: trimmed.blobUrl
                    }));
                }
            } catch (err) {
    ...
    useEffect(() => {
        return () => {
            Object.values(processedProductImages).forEach(url => {
    ...
    // AI RESULT PERSISTENCE - Managed by main debounced effect above
    const restoreLastAiResult = () => {
        try {
            const saved = localStorage.getItem('stylelink_draft_ai_result');
            if (saved && setAiResult) setAiResult(saved);
        } catch (e) {
            console.warn("Failed to restore prompt AI result", e);
        }
    };
    ...
    */
    ...
    const isHooded = useMemo(() => {
        if (!item.productType) return false;
        const type = item.productType.toLowerCase();
        const name = (product?.name || "").toLowerCase();
        return type.includes('hoodie') || type.includes('softshell') || name.includes('capuche') || name.includes('softshell');
    ...
    const handleAiTryOn = async (userPhoto: string, promptOverride?: string) => {
        ...
        setAiModalOpen(true); // ALWAYS keep it open or open it if it was from preview
        ...
        try {
            setIsGeneratingLocal(true); // START LOADING IMMEDIAETLY
            const preview = await generatePreview();
            setAnimationPreview(preview);
            const resizedUserPhoto = await resizeImage(userPhoto, 800, 0.7, 9 / 16); // MANDATORY 9:16 PORTRAIT (STORY FORMAT)
            ...
            if (onGenerateAi) {
                ...
                if (finalCategory === 'Sans Filtre') {
                    ...
                    // --- DETECT LOGO PLACEMENT & COLOR ---
                    const presets = PLACEMENT_PRESETS[item.productType] || PLACEMENT_PRESETS['default'];
                    const posX = isBack ? item.logoPositionXBack : item.logoPositionXFront;
                    const isHeart = Math.abs(posX - presets.heart.x) < 8;
                    const designLayout = isHeart ? 'heart' : 'center';

                    const activeLogoColor = isBack ? item.activeLogoColorBack : item.activeLogoColorFront;
                    const logoColor = (activeLogoColor && activeLogoColor !== 'original')
                        ? COLOR_NAMES[activeLogoColor] || activeLogoColor
                        : 'original';
            ...
            try {
                ...
                audio.volume = 0.5;
                audio.play().catch(e => console.warn("Auto-play blocked by browser. Interaction required."));
            } catch (e) {
    ...
    const capturePhoto = () => {
        if (videoRef.current) {
            ...
            if (ctx) {
                ...
                // --- OPTIMIZATION: BLOB URL for Captured Image ---
                const dataUrl = canvas.toDataURL('image/jpeg', 0.9);
                try {
                    const blob = dataURLtoBlob(dataUrl);
                    if (blob) {
                        const blobUrl = URL.createObjectURL(blob);
                        // Store the blob URL for UI rendering to avoid DOM bloat/LCP issues
    ...
    const changeProductType = (dir: 'next' | 'prev') => {
        ...
        const newColor = products[newType].images[item.color] ? item.color : Object.keys(products[newType].images)[0];

        const cacheKey = `${newType}_${newColor}`;
        const cachedAiImage = aiVariantCache[cacheKey];
    ...
    const isCatalogueMode = item.productType === 'catalogue';

    // REMOVED useMemo to prevent Stale Closures on handlers (activePanel, item, etc.)
    let mobileTools = [
    { id: 'image', icon: 'fa-image', action: handleImportButtonClick },
    { id: 'measure', icon: 'fa-ruler-combined', action: () => { const next = !isMeasureToolActive; setIsMeasureToolActive(next); if (next) setShowSizeGuide(true); } },
    { id: 'validate', icon: 'fa-check', action: () => setShowValidateOptions(!showValidateOptions) },
    { id: 'text', icon: 'fa-font', action: handleTextButtonClick },
    { id: 'group', icon: 'fa-layer-group', action: () => setIsGrouped(!isGrouped) }
];
if (isCatalogueMode) { mobileTools = mobileTools.filter(tool => tool.id === 'validate' || tool.id === 'measure'); }
    ...
    const handleColorChangeRequest = (newColor: string) => {
        if (aiResult && !hasShownColorWarning) {
    ...
    const confirmColorChange = () => {
        if (targetColor) {
            const cacheKey = `${item.productType}_${targetColor}`;
            const cachedAiImage = aiVariantCache[cacheKey];
            ...
            setIsAiViewVisible(!!cachedAiImage);
            setTargetColor(null);
            setHasShownColorWarning(true); // Don't show again in this session
        }
    ...
    return (
        <div className={`flex flex-col min-h-[100dvh] lg:h-full w-full max-w-[1025px] mx-auto relative lg:pb-24 bg-transparent lg:overflow-visible justify-start ${!isCatalogueMode ? 'overflow-hidden' : ''}`} style={{ touchAction: 'pan-y' }}>
            ...
            {/* Mobile Selector Header (Full restoration - centered with w-48) */}
            <div data-layout-id="garment-selector-mobile" className="lg:hidden w-full flex flex-col bg-transparent transition-all flex-shrink-0 z-50 min-h-[56px] border-none mb-2">
                <div className="w-full flex items-center justify-center gap-6 py-2 px-4">
                    <button onClick={() => changeProductType('prev')} className="text-orange-500 font-black text-2xl hover:scale-110 transition-transform">
                        <i className="fa-solid fa-chevron-left"></i>
                    </button>
                    <div className="w-48 flex flex-col items-center overflow-hidden">
                        <h2 className="text-lg font-black text-gray-900 uppercase tracking-widest text-center whitespace-nowrap overflow-hidden text-ellipsis w-full">
                            {product.name}
                        </h2>
                        <div className="flex items-center gap-1.5 -mt-0.5">
                            <span className="text-[10px] font-black text-orange-500 uppercase tracking-wider">{product.reference}</span>
                            <div className="w-1 h-1 rounded-full bg-gray-300"></div>
                            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">Personnalisé</span>
                        </div>
                    </div>
                    <button onClick={() => changeProductType('next')} className="text-orange-500 font-black text-2xl hover:scale-110 transition-transform">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 lg:gap-8 items-start lg:mt-0 w-full max-w-[1025px] mx-auto px-4 lg:px-0">
                <div className="lg:col-span-12 space-y-4 order-1">
                    <div className="flex-1 relative flex flex-col items-center justify-center p-0 lg:p-2 bg-transparent lg:bg-white overflow-visible w-full scrollbar-hide min-h-[100vh] lg:min-h-0">
                        ...
                        <div className="mobile-product-container relative w-full aspect-[3/4] max-h-[55vh] lg:max-h-none lg:w-[450px] lg:h-auto lg:aspect-[3/4] flex flex-col items-center justify-center z-10 m-0 mx-auto lg:mt-[-5vh] lg:mb-10 shadow-none lg:shadow-none">
                            ...
                            >
                                ...
                                >
                                    ...
                                    {aiGenerating && (
                                        <div className="absolute inset-0 z-[200] flex flex-col items-center justify-center bg-black/20 backdrop-blur-[2px] rounded-2xl overflow-hidden animate-fade-in pointer-events-none">
                                            <div className="absolute top-10 left-0 right-0 text-center z-[210]">
                                                <h3 className="text-white text-xl font-black uppercase tracking-widest animate-pulse mb-2 shadow-sm">Magie en cours...</h3>
                                                <div className="inline-flex items-center gap-2 bg-black/80 px-4 py-1.5 rounded-full shadow-xl border border-white/10">
                                                    <div className="w-2 h-2 bg-orange-500 rounded-full animate-pulse"></div>
                                                    <span className="text-white text-[10px] font-black uppercase tracking-wider">Traitement IA...</span>
                                                </div>
                                            </div>
                                            {/* Pulsing Orb - Simplified */}
                                            <div className="relative w-32 h-32 z-[210]">
                                                <div className="absolute inset-0 bg-orange-500/30 rounded-full animate-pulsing-orb"></div>
                                                <div className="absolute inset-0 flex items-center justify-center">
                                                    <div className="w-16 h-1 bg-white/40 rounded-full animate-pulse"></div>
                                                </div>
                                            </div>
                                            <div className="mt-4 z-[210]">
                                                <LoadingScreen
                                                    message="Magie IA..."
                                                    transparent={true}
                                                    local={true}
                                    ...
                                    <>
                                        {isAiViewVisible && (isBack ? item.aiImageUrlBack : item.aiImageUrlFront) || (isAiViewVisible && item.aiImageUrl) ? (
                                            <div className="absolute inset-0 z-40 animate-fade-in group">
                                                <img
                                                    src={(isBack ? item.aiImageUrlBack : item.aiImageUrlFront) || item.aiImageUrl || ''}
                                                    className="w-full h-full object-contain pointer-events-none select-none"
                                                    style={
                                                        item.productType === 'hoodie_bcwg008' ? { transform: 'scale(1.35)' } :
                                                            item.productType === 'sweater_f310' ? { transform: 'scale(1.40)' } :
                                                                item.productType === 'tshirt_s140' ? { transform: 'scale(1.45)' } :
                                                                    item.productType === 'tshirt_p68' ? { transform: 'scale(1.35)' } :
                                                                        undefined
                                                    }
                                                    alt="AI Layer"
                                                />
                                                {/* Badge indicateur IA sur le vêtement */}
                                                <div className="absolute top-4 right-4 bg-black/80 backdrop-blur-md text-white text-[10px] font-black px-4 py-2 rounded-full shadow-lg border border-white/10 flex items-center gap-2 animate-fade-in pointer-events-auto cursor-pointer" onClick={() => setIsAiViewVisible(false)}>
                                                    <div className="w-1.5 h-1.5 bg-orange-500 rounded-full animate-pulse"></div>
                                                    MODE IA ACTIF
                                                    <i className="fa-solid fa-xmark ml-1 opacity-60"></i>
                                                </div>
                                            </div>
                                        ) : (
                                            <div className={`relative w-full h-full`}>
                                                <img
                                                    src={processedProductImages[`${item.productType}_${item.color}_${isBack ? 'back' : 'front'}`] || getProxiedUrl(getOptimizedImageUrl(
                                                        (isBack ?
                                                            (product.backImages && (product.backImages[item.color] || Object.values(product.backImages)[0])) || (product.images[item.color] || Object.values(product.images)[0]) // Fallback to Front if Back missing
                                                            :
                                                            (product.images[item.color] || Object.values(product.images)[0])
                                                        )
                                                        , 800
                                                    ))}
                                                    className="w-full h-full object-contain pointer-events-none select-none relative z-30"
                                                    style={
                                                        item.productType === 'hoodie_bcwg008' ? { transform: 'scale(1.35)' } :
                                                            item.productType === 'sweater_f310' ? { transform: 'scale(1.40)' } :
                                                                item.productType === 'tshirt_s140' ? { transform: 'scale(1.45)' } :
                                                    ...
                                                    alt={`product-${isBack ? 'back' : 'front'}-${item.color}`}
                                                    crossOrigin="anonymous"
                                                    loading="eager"
                                                    // @ts-ignore
                                                    fetchpriority="high"
                                                    onError={(e) => {
                                                        console.error("Image load failed for color:", item.color, e.currentTarget.src, isBack ? "BACK View" : "FRONT View");
                                                        if (e.currentTarget.src.includes('weserv.nl') && (import.meta as any).env?.DEV) {
                                                            const originalSrc = isBack ?
                                                ...
                                                />
                                                {/* Magic/Sparkle animations removed */}
                                            </div>
                                        )}

                                        {/* Overlays (Logo & Texts) - Hidden when AI View is active */}
                                        {!isAiViewVisible && (isBack ? item.predefinedLogoUrlBack || item.originalLogoUrlBack : item.predefinedLogoUrlFront || item.originalLogoUrlFront) && (
                                            <DraggableElement
                                                id={isBack ? "logoBack" : "logoFront"} type="logo" item={item} side={isBack ? "Back" : "Front"}
                                                isActive={activeEl === 'logo'} setActive={() => setActiveEl('logo')}
                                                onOpenOptions={() => { setActiveLogoSlot(1); setActivePanel('import'); }} onUpdate={updateItem} onSaveHistory={saveHistory}
                                        ...
                                        {/* Slot 2 Logo */}
                                        {!isAiViewVisible && (isBack ? item.logoBack2?.originalUrl || item.logoBack2?.predefinedUrl : item.logoFront2?.originalUrl || item.logoFront2?.predefinedUrl) && (
                                            <DraggableElement
                                                ...
                                                onOpenOptions={() => { setActiveLogoSlot(2); setActivePanel('import'); }} onUpdate={updateItem} onSaveHistory={saveHistory}
                                                isEditable={true} realHeight={productDimensions?.[item.productType]?.[previewSize]}
                                                showDimensions={isMeasureToolActive}
                                        ...
                                        {/* Slot 3 Logo */}
                                        {!isAiViewVisible && (isBack ? item.logoBack3?.originalUrl || item.logoBack3?.predefinedUrl : item.logoFront3?.originalUrl || item.logoFront3?.predefinedUrl) && (
                                            <DraggableElement
                                                ...
                                                onOpenOptions={() => { setActiveLogoSlot(3); setActivePanel('import'); }} onUpdate={updateItem} onSaveHistory={saveHistory}
                                                isEditable={true} realHeight={productDimensions?.[item.productType]?.[previewSize]}
                                                showDimensions={isMeasureToolActive}
                                        ...
                                        {!isAiViewVisible && (isBack ? item.textBack.text : item.textFront.text) && (
                                            <DraggableElement
                                        ...
                                        {/* Slot 2 Text */}
                                        {!isAiViewVisible && (isBack ? item.textBack2?.text : item.textFront2?.text) && (
                                            <DraggableElement
                                        ...
                                        {/* Slot 3 Text */}
                                        {!isAiViewVisible && (isBack ? item.textBack3?.text : item.textFront3?.text) && (
                                            <DraggableElement
                                ...
                                {/* 3. BARRE DE VUE / ZOOM (Ovale, chevauche le bas du t-shirt) */}
                                <div className="lg:hidden flex justify-center items-center gap-4 bg-white/95 backdrop-blur-md rounded-full shadow-md border border-gray-100 px-6 py-3 -mt-12 relative z-40 mx-auto w-max mb-6">
                                    <button onClick={() => { setIsBack(!isBack); setActiveView(isBack ? 'front' : 'back'); autoCenterGarment(); }} className="text-gray-500 hover:text-orange-600 transition-colors flex items-center gap-2 text-xs font-bold uppercase tracking-wider">
                                        <i className={`fa-solid fa-arrows-rotate text-lg transition-transform ${isBack ? 'rotate-180' : ''}`}></i> {isBack ? 'Dos' : 'Face'}
                                    </button>
                                    <div className="w-[1px] h-6 bg-gray-200 mx-2"></div>
                                    <button onClick={() => setZoomLevel(prev => Math.min(prev + 0.1, 2.0))} className="text-gray-500 hover:text-orange-600 transition-colors w-8 h-8 flex items-center justify-center active:bg-gray-100 rounded-full"><i className="fa-solid fa-plus text-lg"></i></button>
                                    <div className="w-[1px] h-6 bg-gray-200 mx-2"></div>
                                    <button onClick={() => setZoomLevel(prev => Math.max(prev - 0.1, 0.4))} className="text-gray-500 hover:text-orange-600 transition-colors w-8 h-8 flex items-center justify-center active:bg-gray-100 rounded-full"><i className="fa-solid fa-minus text-lg"></i></button>
                                </div>
                            </div>
                        ...
                        {/* 2. PC SIDEBAR MENU - Hidden on Mobile */}
                        <div className="hidden lg:flex w-full justify-center -mt-16 relative z-40">
                            <div
                                data-layout-id="customizer-right-sidebar"
                                className="bg-white/90 backdrop-blur-2xl border border-gray-100 px-6 py-3 rounded-full shadow-[0_10px_30px_-10px_rgba(0,0,0,0.1)] flex flex-row gap-4 items-center hidden lg:flex"
                            >
                        ...
                        {/* 3. SIZE SELECTOR & ADD TO CART (DESKTOP) */}
                        ...
                        <div data-layout-id="size-selector-desktop" id="size-selector-desktop" className="hidden lg:block bg-white p-3 rounded-2xl border border-gray-100 shadow-sm w-full">
                            <div className="mb-3 px-2 flex items-center justify-between">
                                <div className="flex flex-col">
                                    {/* AI ASSOCIATION INDICATOR (Floating Thumbnail) */}
                                    {item.aiImageUrl && (
                                        <div className="flex flex-col items-center gap-2 p-2 bg-orange-50 rounded-2xl border border-orange-100 shadow-sm animate-fade-in mb-4">
                                            <div className="relative w-20 h-24 rounded-lg overflow-hidden border-2 border-orange-500 shadow-md">
                                                <img src={item.aiImageUrl} className="w-full h-full object-cover" alt="Applied AI" />
                                                <button
                                                    onClick={() => { updateItem({ aiImageUrl: null }); setIsAiViewVisible(false); }}
                                                    className="absolute top-1 right-1 w-5 h-5 bg-orange-600 text-white rounded-full flex items-center justify-center text-[10px] shadow-lg hover:bg-orange-700 transition-colors"
                                                >
                                                    <i className="fa-solid fa-xmark"></i>
                                                </button>
                                            </div>
                                            <span className="text-[9px] font-black text-orange-600 uppercase tracking-widest">Design IA Appliqué</span>
                                        </div>
                                    )}
                        ...
                        {aiResult && (
                            <div className="hidden lg:flex w-full flex-col items-center gap-3 mt-4 mb-2 animate-fade-in">
                                <div
                                    onClick={() => setShowAiResultModal(true)}
                                    className="w-32 h-32 rounded-2xl border-2 border-orange-500 overflow-hidden shadow-2xl cursor-pointer transform hover:scale-110 transition-all relative group bg-white"
                                >
                                    <img src={aiResult.startsWith('blob:') || aiResult.startsWith('data:') ? aiResult : getProxiedUrl(getOptimizedImageUrl(aiResult, 300))} className="w-full h-full object-cover" alt="AI Preview" />
                    ...
                    <div className="lg:hidden w-full px-4 mt-auto mb-6 flex flex-col gap-4 relative z-50">
                        ...
                        {/* 4. BARRE D'OUTILS PRINCIPALE (Mobile) */}
                        <div className="lg:hidden w-full flex justify-between items-center px-4 py-4 mb-6 relative z-30">
                            {mobileTools.map(tool => (
                                <button 
                                    ...
                                    className={`flex items-center justify-center transition-all ${
                                        tool.id === 'validate' 
                                        ? 'bg-green-500 text-white rounded-full p-4 shadow-xl shadow-green-500/30 w-[72px] h-[72px] scale-110 z-50 active:scale-95' 
                                        : 'bg-white border border-gray-100 text-gray-500 rounded-2xl w-14 h-14 shadow-sm hover:border-orange-300'
                                    }`}
                                >
                                    <i className={`fa-solid ${tool.icon} ${tool.id === 'validate' ? 'text-3xl' : 'text-xl'}`}></i>
                                </button>
                            ))}
                        </div>
                        ...
                        {/* 3. MOBILE SIZE SELECTOR (Mobile Only) */}
                        ...
                        {(aiGenerating || aiResult) && (
                            <div className="w-full bg-white/80 backdrop-blur-md rounded-2xl border border-white/20 shadow-lg px-4 py-3 flex flex-col gap-2 lg:hidden mt-2">
                                ...
                                {aiGenerating ? (
                                    <div className="flex items-center justify-center p-4 bg-orange-50 rounded-xl border border-orange-100">
                                    ...
                                    <div
                                        onClick={() => setShowAiResultModal(true)}
                                        className="relative w-full aspect-square rounded-xl overflow-hidden border-2 border-orange-500 cursor-pointer group shadow-sm bg-gray-50"
                                    >
            ...
            {/* AI COLOR CHANGE WARNING MODAL */}
            {
                showColorWarning && (
                    <div className="fixed inset-0 z-[10002] bg-black/80 backdrop-blur-sm flex items-center justify-center p-6 animate-fade-in">
                        <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl relative">
                            <div className="w-16 h-16 bg-orange-100 rounded-full flex items-center justify-center mx-auto mb-4">
                                <i className="fa-solid fa-triangle-exclamation text-orange-600 text-3xl"></i>
                            </div>
                            <h3 className="text-xl font-black text-center text-gray-900 mb-3">Attention !</h3>
                            <p className="text-center text-gray-600 mb-4 px-2">
                                L'image IA actuelle est liée à cette couleur. Si vous changez, elle sera réinitialisée.
                            </p>
                            <div className="bg-blue-50 border border-blue-100 p-3 rounded-lg mb-6">
                                <p className="text-xs text-blue-800 text-center font-bold">
                                    <i className="fa-solid fa-circle-info mr-1"></i>
                                    Pour conserver cette version, veuillez l'ajouter au panier ou <span className="underline">l'enregistrer dans votre profil</span> (connexion requise).
                                </p>
                            </div>
            ...
            <div className="tools-portals-container">
                ...
                {showAiResultModal && aiResult && createPortal(
                    <div className="fixed inset-0 z-[10001] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fade-in">
                        <div className="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl relative flex flex-col items-center p-6 max-h-[90vh] overflow-y-auto">
                            <button
                                onClick={() => setShowAiResultModal(false)}
                                className="absolute top-4 right-4 w-10 h-10 flex items-center justify-center bg-gray-100 rounded-full text-gray-500 hover:bg-gray-200 z-10"
                            >
                                <i className="fa-solid fa-xmark text-lg"></i>
                            </button>

                            <h3 className="text-xl font-black text-gray-900 mb-6 uppercase tracking-tight">VOTRE CRÉATION IA</h3>

                            <div className="w-full aspect-square rounded-2xl overflow-hidden border border-gray-100 shadow-inner mb-8 bg-gray-50">
                                <img src={aiResult} className="w-full h-full object-contain" alt="AI HD" />
                            </div>

                            <div className="flex flex-col w-full gap-3">
                                <button
                                    onClick={() => { setCapturedImage(null); setShowAiResultModal(false); }}
                                    className="w-full py-4 bg-green-600 text-white font-black uppercase tracking-widest rounded-2xl shadow-xl hover:bg-green-700 transition-all flex items-center justify-center gap-2"
                                >
                                ...
                                <button
                                    ...
                                    className="w-full py-4 bg-white text-gray-500 font-bold rounded-2xl border border-gray-200 hover:text-red-600 hover:border-red-100 hover:bg-red-50 transition-all flex items-center justify-center gap-2"
                                >
                                ...
                                <button
                                    onClick={async () => {
                                        if (!user) {
                                            onAuthRequired();
                                            // We keep the modal open so they can save after login (parent handles "post-login" logic if needed, or user clicks again)
                                        } else {
                                            ...
                                            if (btn) {
                                                ...
                                                try {
                                                    await onSaveToProfile(aiResult, selectedAiPrompt, selectedStyleName, item);
                                    ...
                                    id="btn-save-profile"
                                    className="w-full py-4 bg-gray-100 text-gray-600 font-bold rounded-2xl border border-gray-200 hover:bg-orange-50 hover:text-orange-600 hover:border-orange-200 transition-all flex items-center justify-center gap-2"
                                >
                    ...
                    activePanel === 'import' && createPortal(
                        <div data-layout-id="desktop-panel-import" className="fixed inset-0 z-[9999] flex items-end lg:items-start justify-center lg:justify-end bg-transparent lg:pt-16 p-0 lg:p-4 pointer-events-none" style={{ display: 'flex', opacity: 1, visibility: 'visible', top: 0 }}>
                            ...
                            >
                                ...
                                {/* MULTI-LOGO SLOT SELECTOR */}
                                <div className="flex items-center gap-2 mb-3 no-drag">
                                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-wider shrink-0">Slot</span>
                                    <div className="flex gap-1.5 flex-1">
                                        {([1, 2, 3] as const).map(slot => {
                                            const hasContent = slot === 1
                                                ? !!(isBack ? (item.originalLogoUrlBack || item.predefinedLogoUrlBack) : (item.originalLogoUrlFront || item.predefinedLogoUrlFront))
                                                : slot === 2
                                                    ? !!(isBack ? item.logoBack2?.originalUrl : item.logoFront2?.originalUrl)
                                                    : !!(isBack ? item.logoBack3?.originalUrl : item.logoFront3?.originalUrl);
                                            return (
                                                <button
                                                    key={slot}
                                                    onClick={() => setActiveLogoSlot(slot)}
                                                    className={`flex-1 h-8 rounded-xl border-2 text-xs font-black transition-all flex items-center justify-center gap-1.5 ${activeLogoSlot === slot
                                                ...
                                                >
                                                    {hasContent && (
                                                        <span className={`w-1.5 h-1.5 rounded-full ${activeLogoSlot === slot ? 'bg-white' : 'bg-orange-400'}`}></span>
                                                    )}
                                                    Logo {slot}
                                                </button>
                                            );
                                ...
                                {(() => {
                                    // Compute logoData dynamically for the active slot
                                    let logoData: {
                                    ...
                                    if (activeLogoSlot === 1) {
                                        ...
                                        const s2 = isBack ? item.logoBack2 : item.logoFront2;
                                        logoData = {
                                            originalUrl: s2?.originalUrl,
                                            predefinedUrl: s2?.predefinedUrl,
                                            processedUrl: s2?.processedUrl,
                                            processedUrl_original: s2?.processedUrl_original,
                                            backgroundRemoved: s2?.backgroundRemoved,
                                            activeColor: s2?.activeColor,
                                            inverted: s2?.inverted
                                        };
                                    } else {
                                        const s3 = isBack ? item.logoBack3 : item.logoFront3;
                                        logoData = {
                                            originalUrl: s3?.originalUrl,
                                            predefinedUrl: s3?.predefinedUrl,
                                            processedUrl: s3?.processedUrl,
                                            processedUrl_original: s3?.processedUrl_original,
                                            backgroundRemoved: s3?.backgroundRemoved,
                                            activeColor: s3?.activeColor,
                                            inverted: s3?.inverted
                                        };
                                    }
                                                    ...
                                                    <button
                                                        onClick={() => {
                                                            ...
                                                            if (activeLogoSlot === 1) {
                                                                ...
                                                                cleanup = { [isBack ? 'logoBack2' : 'logoFront2']: undefined };
                                                            } else if (activeLogoSlot === 3) {
                                                                cleanup = { [isBack ? 'logoBack3' : 'logoFront3']: undefined };
                                                            }
                                                            updateItem(cleanup);
                                                            setActiveEl(null);
                                                            setUploadedLogoPreview(null);
                                            ...
                                            {codeLogoPreview && (
                                                <div className="flex flex-col items-center gap-2 flex-shrink-0">
                                                    <div className="h-24 w-24 bg-gray-100 border border-gray-300 rounded-lg p-2 flex items-center justify-center relative">
                                                        <img src={getProxiedUrl(codeLogoPreview, { width: 150 })} className="max-w-full max-h-full object-contain" alt="Selected Code Logo" />
                                                    </div>
                    ...
                    aiModalOpen && createPortal(
                        <div data-layout-id="ai-modal-fullscreen" className="fixed inset-0 h-[100dvh] z-[10000] bg-white animate-fade-in flex flex-col overflow-hidden">
                            ...
                            <div className="flex-1 overflow-y-auto overflow-x-hidden relative bg-white">
                                                ...
                                                >
                                                    {allStyles.map((item, idx) => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                <LazyImage
                                                                    src={getProxiedUrl(item.style.image, { width: 300, quality: 70 })}
                                                                    alt={item.style.name}
                                                                    loading="lazy"
                                ...
                                {/* STEP 2: INPUT METHOD (Camera / Gallery) */}
                                {aiStep === 'input' && (
                                    <div className="h-full flex flex-col items-center justify-center p-6 animate-fade-in">
                                        ...
                                        {!capturedImage ? (
                                            <div className="w-full max-w-none lg:max-w-sm flex flex-col gap-6 items-center">
                                                ...
                                                <div className="flex-1 relative rounded-2xl overflow-hidden shadow-xl bg-black">
                                                    ...
                                                    {isBack && (
                                                        <div className="absolute bottom-4 left-4 right-4 z-40">
                                                            <div className="bg-orange-500/90 backdrop-blur-md p-3 rounded-xl border border-white/20 shadow-lg animate-bounce">
                                                                <div className="flex items-center gap-3 text-white">
                                                                    <i className="fa-solid fa-circle-exclamation text-xl"></i>
                                                                    <div className="flex-1">
                                                                        <p className="text-[10px] font-black uppercase tracking-widest leading-none">Attention : Vue de Dos</p>
                                                                        <p className="text-[9px] font-bold opacity-80 mt-1">Ton vêtement est de dos. Pour un meilleur résultat, utilise une photo de ton dos ou un mannequin de dos.</p>
                                                                    </div>
                                                                    <button
                                                                        onClick={(e) => { e.stopPropagation(); handleAiTryOn(POSE_IMAGES.back); }}
                                                                        className="bg-white text-orange-600 px-3 py-2 rounded-lg text-[9px] font-black uppercase whitespace-nowrap hover:bg-orange-50 transition-all shadow-sm"
                                                                    >
                                                                        Mannequin Dos
                                                                    </button>
                                                                </div>
                                                    ...
                                                    {(aiGenerating || isGeneratingLocal) && (
                                                        <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/60 backdrop-blur-md">
                                                            <div className="w-full h-full p-4">
                                                                ...
                                                                <div className="absolute inset-0 flex items-center justify-center">
                                                                    <LoadingScreen transparent local message="Magie en cours..." />
                                                                </div>
                                                            </div>
                                                        </div>
                                                    )}
                                        ...
                                        {(aiGenerating || isGeneratingLocal) ? (
                                            <div className="flex-1 flex flex-col items-center justify-center relative overflow-hidden">
                                                ...
                                                <div className="relative z-10 flex flex-col items-center justify-center w-full h-full">
                                                    ...
                                                    <div className="w-full h-full flex items-center justify-center p-8">
                                                        <div className="relative w-full h-full max-w-lg aspect-[9/16] flex items-center justify-center">
                                                            {/* Garment Preview with Magic Animation */}
                                                            <div className="relative w-full h-full">
                                                                <img
                                                                    src={animationPreview || processedProductImages[`${item.productType}_${item.color}_${isBack ? 'back' : 'front'}`]}
                                                                    className="w-full h-full object-contain relative z-30"
                                                                    alt="Loading Garment"
                                                                />

                                                                {/* Magic effects removed, keeping garment glow if acceptable */}
                                                                <div className="absolute inset-0 z-40 pointer-events-none overflow-hidden">
                                                                </div>
                                                            </div>

                                                            {/* Simple Loading Message removed dots bounce as requested */}
                                                            <div className="absolute inset-0 z-50 flex items-center justify-center pointer-events-none">
                                                            </div>
                                                        </div>

                                                        {/* Global Loading Overlay (Steps) */}
                                                        <div className="absolute inset-0 z-[60] flex items-end pb-20 justify-center pointer-events-none">
                                                            <LoadingScreen
                                                                message="Création de ta variante..."
                                                                steps={["Analyse stylistique", "Fusion avec le vêtement", "Génération Haute Qualité"]}
                                                                currentStep={1}
                                                                transparent={true}
                                                                local={true}
                                                            />
                                            ...
                                            <div className="flex-1 flex flex-col relative animate-fade-in">
                                                <div className="flex-1 bg-black relative flex items-center justify-center">
                                                    {/* Full-Text Action Toolbar (Right) */}
                                                    <div className="absolute right-8 top-1/2 -translate-y-1/2 flex flex-col gap-4 z-50 w-64">
                                                        <button
                                                            onClick={() => {
                                                                if (aiResult) {
                                                                    const cacheKey = `${item.productType}_${item.color}`;
                                                                    setAiVariantCache(prev => ({
                                                                        ...prev,
                                                                        [cacheKey]: aiResult
                                                                    }));
                                                                    updateItem({
                                                                        aiImageUrl: aiResult,
                                                                        ...(isBack ? { aiImageUrlBack: aiResult } : { aiImageUrlFront: aiResult })
                                                                    });
                                                                    setIsAiViewVisible(true);
                                                                    setAiModalOpen(false);
                                                                }
                                                            }}
                                                            className="w-full py-5 bg-orange-600 text-white rounded-2xl shadow-xl flex items-center justify-center gap-3 hover:bg-orange-500 hover:scale-[1.02] active:scale-95 transition-all border border-white/20 group"
                                                        >
                                                            <span className="font-black uppercase tracking-widest text-sm">GARDER CE MODÈLE</span>
                                                            <i className="fa-solid fa-check group-hover:translate-x-1 transition-transform"></i>
                                                        </button>

                                                        <button
                                                            onClick={() => { setAiResult && setAiResult(null); setAiStep('input'); }}
                                                            className="w-full py-4 bg-white/10 backdrop-blur-md border border-white/20 text-white rounded-2xl flex items-center justify-center gap-3 hover:bg-white/20 hover:scale-[1.02] active:scale-95 transition-all text-xs font-bold uppercase tracking-widest"
                                                        >
                                                        ...
                                                        <button
                                                            onClick={async () => {
                                                                if (!user) {
                                                                    ...
                                                                    const btn = document.getElementById('btn-save-profile-studio-full');
                                                                    if (btn) {
                                                                        ...
                                                                        try {
                                                                            ...
                                                                            btn.innerHTML = '<span>SAUVEGARDÉ !</span> <i class="fa-solid fa-heart"></i>';
                                                                            ...
                                                                            btn.innerHTML = '<span>ERREUR</span> <i class="fa-solid fa-triangle-exclamation"></i>';
                                                                            setTimeout(() => { if (btn) btn.innerHTML = originalContent; }, 2000);
                                                                        }
                                                                    }
                                                                }
                                                            }}
                                                            id="btn-save-profile-studio-full"
                                                            className="w-full py-4 bg-transparent border border-white/30 text-white/50 rounded-2xl shadow-lg flex items-center justify-center gap-3 hover:bg-white/10 hover:text-white hover:scale-[1.02] active:scale-95 transition-all"
                                                        >
                                                            <span className="font-black uppercase tracking-widest text-xs">AJOUTER À MA GALERIE</span>
                                                            <i className={`fa-solid ${user ? 'fa-heart' : 'fa-lock'}`}></i>
                                                        </button>
                                                    </div>

                                                    {/* Background Blur Overlay for the Large image */}

                                                    <div className="absolute inset-0 overflow-hidden">
                                                        <img src={aiResult} className="w-full h-full object-cover blur-[80px] opacity-60 scale-110" alt="Background Blur" />
                                                        <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-black/40"></div>
                                                    </div>

                                                    <img src={aiResult} className="relative z-20 max-w-full max-h-full object-contain drop-shadow-[0_20px_50px_rgba(0,0,0,0.5)]" alt="AI Result" />
                                                </div>

                                                <div className="flex-none bg-black/80 backdrop-blur-md p-4 pb-12 flex flex-col items-center justify-center z-10 border-t border-white/10">
                                                    <div className="inline-flex items-center gap-2 bg-orange-600 px-4 py-2 rounded-full shadow-lg border border-orange-500/50">
                                                        <i className="fa-solid fa-fire text-white text-xs animate-pulse"></i>
                                                        <span className="text-white text-[10px] font-black uppercase tracking-widest">Design généré !</span>
                                                    </div>
                                                </div>
                                            </div>
                                        ) : null}
                ...
                {/* TRUE FULLSCREEN PREVIEW OVERLAY - HIGHEST Z-INDEX */}
                {
                    previewImage && createPortal(
                        <div className="fixed inset-0 z-[10000] bg-black/90 backdrop-blur-sm animate-fade-in flex items-center justify-center p-0 lg:p-8">
                            <div className="relative w-full max-w-md h-full lg:max-h-[85vh] lg:rounded-3xl bg-black shadow-2xl overflow-hidden flex flex-col">
                                    ...
                                    <div className="absolute inset-0 z-[10002] flex flex-col items-center justify-center bg-black/40 backdrop-blur-md">
                                        <div className="absolute top-20 w-full text-center z-20">
                                            ...
                                            <p className="text-[10px] text-white/90 font-bold uppercase tracking-widest bg-orange-600 px-4 py-2 rounded-full inline-block shadow-xl">
                                                Génération de Crédits
                                            </p>
                                        </div>
                                        <div className="w-full h-full">
                                            <LoadingScreen
                                                message="Un instant..."
                                                steps={["Préparation de la scène", "Upload sécurisé", "Démarrage IA"]}
                                                currentStep={0}
                                            />
                                        </div>
                                    </div>
                ...
                {/* MOBILE STICKY PRICE & ACTION BAR */}
                {
                    window.innerWidth < 1024 && activePricing.total > 0 && !previewImage && !aiGenerating && (
                        <div className="relative mt-auto border-t border-gray-100 bg-white px-4 py-8 w-full flex flex-col gap-4 lg:hidden">
                            <div className="flex flex-col gap-4 max-w-lg mx-auto w-full">
                                ...
                                <div className="flex flex-col gap-3 w-full mt-4">
                                    <button
                                        onClick={handleShareDesign}
                                        disabled={sharingLoading}
                                        className="w-full py-4 bg-gray-100 hover:bg-gray-200 text-gray-900 font-bold rounded-xl transition-all flex items-center justify-center gap-2"
                                    >
                                        <i className={`fa-solid ${sharingLoading ? 'fa-spinner fa-spin' : 'fa-share-nodes'}`}></i>
                                    ...
                                    <button
                                        ...
                                        className="w-full py-4 bg-orange-600 hover:bg-orange-500 text-white font-bold rounded-xl shadow-lg shadow-orange-500/20 transition-all flex items-center justify-center gap-2"
                                    >
