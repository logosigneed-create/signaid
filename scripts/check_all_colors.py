...
colors = ["Black", "Graphite Black", "Bone", "Clay", "Heather Grey", "Light Olive", "Midnight Navy", "Shitake", "Stonewash Denim", "Tan", "White"]

for col in colors:
    var_url = f'https://shop.l-shop-team.be/index.php?lang=8&cl=oxpsvariantsfetch&parentid=2484756&color={urllib.parse.quote(col)}'
    req2 = urllib.request.Request(var_url, headers=headers)
    with opener.open(req2) as resp:
        data = resp.read().decode('utf-8', errors='ignore')
        res = json.loads(data)
        pic = res.get('picture', {})
        print(f"Color: {col:15} -> url: {pic.get('url')} | hasMore: {pic.get('hasMore')}")
