...
async function main() {
    ...
    if (!email || !password) {
        console.error('❌ Usage: node set_light_theme.mjs <email> <password>');
        process.exit(1);
    }
    ...
    console.log(`☀️ Setting theme to 'light' on SiteConfigs/${TARGET_UID}...`);
    const siteConfigRef = doc(db, 'SiteConfigs', TARGET_UID);
    await updateDoc(siteConfigRef, {
        theme: 'light'
    });
    console.log(`✅ Successfully set theme to 'light'!`);
    process.exit(0);
}

main().catch(err => {
    console.error("❌ Error setting light theme:", err);
    process.exit(1);
});
