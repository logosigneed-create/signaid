...
<head>
  <meta charset="UTF-8">
  <title>Éditeur Vectoriel - Curseur d'Arrondi des Courbes</title>
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Inter, sans-serif; }}
    ...
    .curve-slider-box {{ background: #190924; border: 1px solid #ec4899; padding: 12px; border-radius: 8px; display: flex; flex-direction: column; gap: 8px; margin-top: 4px; }}
    ...
    textarea {{ width: 100%; height: 110px; background: #090d16; color: #38bdf8; border: 1px solid #334155; padding: 10px; font-family: monospace; font-size: 0.75rem; border-radius: 6px; resize: none; }}
...
<body>
  ...
  <header>
    <h1>🎚️ Éditeur Vectoriel - Curseur de Réglage de l'Arrondi des Courbes</h1>
    <div class="toolbar">
  ...
  <div class="main">
    ...
    <div class="sidebar">
      <div class="question-card">
        ...
        <!-- Curvature Arch Slider Box (Visible when segment is a curve) -->
        <div id="arch-slider-box" class="curve-slider-box" style="display: none;">
          <label style="color: #f472b6; font-weight: 700;">🎚️ Réglage de l'Arrondi / Galbe : <span id="arch-val">0 px</span></label>
          <input type="range" id="arch-slider" min="-60" max="60" value="0" step="1" oninput="adjustSegmentArch(this.value)">
          <small style="color: #cbd5e1; font-size: 0.7rem;">Déplacez le curseur pour accentuer ou aplanir le galbe entre les 2 points !</small>
        </div>
      </div>
  ...
  <script>
    ...
    let paths = [
      {{
        ...
        nodes: [
          {{ x: 449, y: 30, type: 'L', arch: 0 }},
          {{ x: 386, y: 70, type: 'L', arch: 0 }},
          {{ x: 385, y: 194, type: 'L', arch: 0 }},
          {{ x: 209, y: 243, type: 'L', arch: 0 }},
          {{ x: 209, y: 46, type: 'L', arch: 0 }},
          {{ x: 147, y: 86, type: 'L', arch: 0 }},
          {{ x: 146, y: 301, type: 'L', arch: 0 }},
          {{ x: 449, y: 205, type: 'L', arch: 0 }}
        ]
      }},
      {{
        ...
        nodes: [
          ...
          {{ x: 24, y: 396, type: 'L', arch: 0 }},
          {{ x: 145, y: 353, type: 'L', arch: 0 }},
          {{ x: 147, y: 435, type: 'L', arch: 0 }},
          {{ x: 209, y: 397, type: 'L', arch: 0 }},
          {{ x: 210, y: 325, type: 'L', arch: 0 }},
          {{ x: 385, y: 254, type: 'L', arch: 0 }},
          {{ x: 385, y: 421, type: 'L', arch: 0 }},
          {{ x: 449, y: 382, type: 'L', arch: 0 }},
          {{ x: 449, y: 232, type: 'L', arch: 0 }}
        ]
      }}
    ];
    ...
    // Adjust segment curvature arch dynamically via slider
    ...
    function chooseSegmentType(type) {{
      ...
      if (type === 'L') {{
        ...
        node.type = 'C';
        if (!node.arch || node.arch === 0) node.arch = -15; // default curve arch
    ...
    function updateUI() {{
      ...
      // Show/Hide Curvature Arch Slider depending on segment type
    ...
    function downloadSVG() {{
      ...
      a.download = 'logo_avec_curseur_arrondi.svg';
      a.click();
    }}
...
print(f"Arch slider vector editor saved to {output_path}")
