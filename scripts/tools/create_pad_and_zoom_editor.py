...
<head>
  <meta charset="UTF-8">
  <title>Éditeur Vectoriel - Zoom Haute Précision & Pad 2D (Axe X/Y)</title>
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Inter, sans-serif; }}
    ...
    #viewport-wrapper {{ position: relative; width: 581px; height: 481px; transform-origin: center center; transition: transform 0.05s ease-out; }}
    ...
    .pad-container {{ background: #0c1829; border: 1px solid #38bdf8; padding: 10px; border-radius: 8px; display: flex; flex-direction: column; gap: 8px; align-items: center; }}
    .pad-area {{ width: 140px; height: 140px; background: #090d16; border: 2px solid #334155; border-radius: 12px; position: relative; cursor: crosshair; touch-action: none; overflow: hidden; display: flex; justify-content: center; align-items: center; }}
    ...
    .pad-knob {{ width: 22px; height: 22px; background: #38bdf8; border: 2px solid #ffffff; border-radius: 50%; position: absolute; transform: translate(-50%, -50%); top: 50%; left: 50%; box-shadow: 0 0 10px rgba(56, 189, 248, 0.8); cursor: pointer; }}

    .dpad-grid {{ display: grid; grid-template-columns: repeat(3, 34px); grid-template-rows: repeat(3, 34px); gap: 4px; justify-content: center; }}
    .dpad-btn {{ background: #1e293b; border: 1px solid #334155; color: #cbd5e1; border-radius: 6px; font-size: 0.9rem; cursor: pointer; display: flex; justify-content: center; align-items: center; font-weight: bold; }}
    .dpad-btn:hover {{ background: #38bdf8; color: #0f1117; }}
...
<body>
  ...
  <header>
    <h1>🔍 Éditeur Vectoriel - Zoom Précision & Pad 2D Multidirectionnel (Axe X/Y)</h1>
    <div class="toolbar">
  ...
  <div class="main">
    ...
    <div class="sidebar">
      ...
      <!-- 2D Trackpad Pad (Axe X & Y) Card -->
      <div class="pad-container">
        <div style="font-size: 0.8rem; font-weight: 700; color: #38bdf8; width: 100%; display: flex; justify-content: space-between;">
          <span>🕹️ Pad 2D Multidirectionnel (X/Y)</span>
          <span id="coords-display" style="color: #fbbf24;">X: 0, Y: 0</span>
        </div>
        ...
        <!-- Micro-Step D-Pad Arrow Buttons -->
        <div class="dpad-grid">
          <div></div>
          <button class="dpad-btn" onclick="stepMove(0, -1)">▲</button>
          <div></div>
          <button class="dpad-btn" onclick="stepMove(-1, 0)">◄</button>
          <button class="dpad-btn" onclick="resetPadPosition()">●</button>
          <button class="dpad-btn" onclick="stepMove(1, 0)">►</button>
          <div></div>
          <button class="dpad-btn" onclick="stepMove(0, 1)">▼</button>
          <div></div>
        ...
        <!-- Angle Controls for Straight Lines -->
        <div id="straight-angle-box" style="width: 100%; display: flex; flex-direction: column; gap: 4px;">
          <label style="color: #38bdf8; font-weight: 700; font-size: 0.75rem;">📐 Angle: <span id="angle-val">90°</span></label>
  ...
  <script>
    ...
    // Mouse wheel zoom
    document.getElementById('canvas-container').onwheel = (e) => {{
      e.preventDefault();
    ...
    function handlePadMove(e) {{
      ...
      // Map Pad coordinates (-50 to +50 delta px)
      let deltaX = Math.round(((mouseX / rect.width) - 0.5) * 80);
      let deltaY = Math.round(((mouseY / rect.height) - 0.5) * 80);

      document.getElementById('coords-display').innerText = `X: ${{deltaX}}, Y: ${{deltaY}}`;
    ...
    function resetPadPosition() {{
      padKnob.style.left = '50%';
      padKnob.style.top = '50%';
      document.getElementById('coords-display').innerText = 'X: 0, Y: 0';
    }}
    ...
    function downloadSVG() {{
      ...
      a.download = 'logo_pad2d_zoom.svg';
      a.click();
    }}
...
print(f"2D Trackpad & Zoom vector editor saved to {output_path}")
