...
<head>
  <meta charset="UTF-8">
  <title>Éditeur Vectoriel - Gestion Étape par Étape (Droites & Courbes)</title>
  <style>
...
<body>
  ...
  <header>
    <h1>🎯 Assistant Vectoriel - Gestion Étape par Étape (Droites & Courbes)</h1>
    <div class="toolbar">
    ...
    <div class="sidebar">
      ...
      <div class="step-card">
        ...
        <div class="step-desc" id="step-desc">
          Ajustez le point et choisissez le type de segment (Droite L ou Courbe C).
        </div>

        <div style="font-size: 0.75rem; color: #94a3b8; font-weight: 700; margin-top: 4px;">TYPE DE SEGMENT :</div>
        <div class="type-switcher">
          <button id="btn-type-L" class="type-btn" onclick="setSegmentType('L')">📏 Ligne Droite (L)</button>
  ...
  <script>
    ...
    let allSteps = [
      {{ pIdx: 0, nIdx: 0, label: "Point 1: Biseau Haut-Droit", desc: "Segment entre Sommet Droit et Sommet Gauche." }},
      {{ pIdx: 0, nIdx: 1, label: "Point 2: Biseau Haut-Gauche", desc: "Coin haut du pilier droit." }},
      {{ pIdx: 0, nIdx: 2, label: "Point 3: Début Entaille Supérieure", desc: "Contrôlez l'entaille transversale." }},
      {{ pIdx: 0, nIdx: 3, label: "Point 4: Fin Entaille Supérieure", desc: "Fin de l'entaille transversale." }},
      {{ pIdx: 0, nIdx: 5, label: "Point 5: Biseau Haut Gauche", desc: "Sommet haut du pilier gauche." }},
      {{ pIdx: 0, nIdx: 6, label: "Point 6: Bord Gauche Pilier", desc: "Bord extérieur gauche." }},
      {{ pIdx: 0, nIdx: 7, label: "Point 7: Tranchée Supérieure", desc: "Contrôlez la courbe transversale haut." }},
      
      {{ pIdx: 1, nIdx: 0, label: "Point 8: Pointe Extrême Droite de la Lame", desc: "Pointe acérée à droite." }},
      {{ pIdx: 1, nIdx: 1, label: "Point 9: Lame Principale", desc: "Courbe principale de la blade." }},
      {{ pIdx: 1, nIdx: 2, label: "Point 10: Pointe Extrême Gauche de la Lame", desc: "Pointe acérée en bas à gauche." }},
      {{ pIdx: 1, nIdx: 3, label: "Point 11: Sommet Pilier Bas Gauche", desc: "Jonction de la lame et du pilier." }},
      {{ pIdx: 1, nIdx: 4, label: "Point 12: Bas du Pilier Gauche", desc: "Biseau bas du pilier gauche." }},
      {{ pIdx: 1, nIdx: 5, label: "Point 13: Intérieur Pilier Bas Gauche", desc: "Coin intérieur bas gauche." }},
      {{ pIdx: 1, nIdx: 6, label: "Point 14: Entaille Inférieure", desc: "Contrôlez l'entaille basse." }},
      {{ pIdx: 1, nIdx: 7, label: "Point 15: Intérieur Pilier Bas Droit", desc: "Coin intérieur bas droit." }},
      {{ pIdx: 1, nIdx: 8, label: "Point 16: Bas du Pilier Droit", desc: "Biseau bas du pilier droit." }},
      {{ pIdx: 1, nIdx: 9, label: "Point 17: Haut du Pilier Bas Droit", desc: "Jonction du pilier bas droit." }}
    ];
    ...
    function validateCurrentStep() {{
      ...
      if (currentStepIndex < allSteps.length - 1) {{
        ...
        alert("🎉 Félicitations ! Tous les segments (droites et courbes) ont été configurés et validés !");
      }}
      render();
    }}
...
print(f"Segment type step-by-step editor saved to {output_path}")
