...
<head>
  <meta charset="UTF-8">
  <title>Éditeur Vectoriel - Assistant de Décision Droite / Courbe</title>
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Inter, sans-serif; }}
    ...
    .question-card {{ background: #0f1117; padding: 16px; border-radius: 10px; border: 2px solid #38bdf8; display: flex; flex-direction: column; gap: 12px; }}
    .question-header {{ display: flex; justify-content: space-between; font-weight: 700; color: #38bdf8; font-size: 0.95rem; }}
    .question-prompt {{ font-size: 0.85rem; color: #f8fafc; font-weight: 600; line-height: 1.4; background: #1e293b; padding: 10px; border-radius: 6px; border-left: 3px solid #fbbf24; }}
    ...
    .draft-path {{ fill: rgba(255, 255, 255, 0.85); stroke: #334155; stroke-width: 1.5px; }}
    
    .active-segment-highlight {{ stroke: #fbbf24; stroke-width: 5px; stroke-dasharray: 6 3; animation: dash 1s linear infinite; }}
    @keyframes dash {{
...
<body>
  ...
  <header>
    <h1>❓ Assistant de Décision - Droite ou Courbe entre 2 Points</h1>
    <div class="toolbar">
    ...
    <div class="sidebar">
      ...
      <div class="question-card">
        <div class="question-header">
          ...
          <span id="segment-type-badge" style="color: #fbbf24; font-size: 0.8rem;">En cours...</span>
        </div>
        <div class="question-prompt" id="question-text">
          Entre le <b>Point A (Bleu)</b> et le <b>Point B (Rouge)</b>, s'agit-il d'une ligne droite ou d'une courbe ?
        </div>
  ...
  <script>
    ...
    function chooseSegmentType(type) {{
      ...
      if (currentSegmentIndex < allSegments.length - 1) {{
        ...
        alert("🎉 Bravo ! Tous les segments ont été vérifiés et configurés !");
      }}
      render();
    }}
    ...
    function updateUI() {{
      ...
      document.getElementById('question-text').innerHTML = `<b>${{seg.label}}</b><br><small>${{seg.desc}}</small><br><br>Entre le <b>Point A (Bleu)</b> et le <b>Point B (Rouge)</b>, s'agit-il d'une Ligne Droite ou d'une Courbe ?`;
    }}
    ...
    function render() {{
      ...
      // Highlight active segment between Point A and Point B
    ...
    function downloadSVG() {{
      ...
      a.download = 'logo_interactif_droite_courbe.svg';
      a.click();
    }}
...
print(f"Segment decision wizard editor saved to {output_path}")
