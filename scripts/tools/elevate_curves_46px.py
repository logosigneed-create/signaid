...
# SVG with Curves elevated to 46 px arch height:
perfected_svg_code = '''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 581 481" width="100%" height="100%">
  <!-- Calque Vectoriel 100% Transparent (Sans Fond) - Courbes Élevées à 46px -->
  <g fill="#ffffff">
    <!-- Forme Supérieure (avec Courbe de Lame Élevée à 46px: C 232,225 332,194 449,205) -->
    <path d="M 449,30 L 386,70 L 385,194 L 209,243 L 209,46 L 147,86 L 146,301 C 232,225 332,194 449,205 Z" />
    <!-- Forme Inférieure (avec Courbes de Lame Élevées à 46px) -->
    <path d="M 569,201 C 404,308 224,373 23,396 C 42,338 88,325 145,353 L 147,435 L 209,397 L 210,325 L 385,254 L 385,425 L 449,382 L 449,232 Z" />
  </g>
</svg>'''
...
print(f"Updated logo.svg with 46px elevated curves: {svg_workspace_path}")

html_content = f'''<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="UTF-8">
  <title>Éditeur Vectoriel - Courbes Élevées à 46 px</title>
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Inter, sans-serif; }}
    ...
    header {{ background: #1e293b; padding: 10px 20px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #334155; }}
    header h1 {{ font-size: 1.05rem; font-weight: 600; color: #38bdf8; display: flex; align-items: center; gap: 8px; }}
    
    .toolbar {{ display: flex; gap: 8px; align-items: center; }}
    .btn {{ background: #2563eb; color: white; border: none; padding: 6px 12px; border-radius: 6px; cursor: pointer; font-weight: 500; font-size: 0.82rem; transition: all 0.2s; }}
    .btn:hover {{ background: #1d4ed8; transform: translateY(-1px); }}
    .btn-secondary {{ background: #334155; }}
    .btn-secondary:hover {{ background: #475569; }}
    .btn-magic {{ background: linear-gradient(135deg, #10b981, #6366f1); color: white; font-weight: 700; font-size: 0.88rem; padding: 8px 16px; box-shadow: 0 0 12px rgba(16, 185, 129, 0.5); }}
    .btn-magic:hover {{ filter: brightness(1.15); transform: translateY(-1px); }}
    .btn-straight {{ background: #0284c7; color: white; font-weight: 700; font-size: 0.85rem; padding: 8px; }}
    .btn-curve {{ background: #db2777; color: white; font-weight: 700; font-size: 0.85rem; padding: 8px; }}
    .btn-active {{ background: #10b981; color: #ffffff; font-weight: 700; font-size: 0.88rem; }}

    .main {{ display: flex; flex: 1; overflow: hidden; }}
    .canvas-container {{ flex: 1; position: relative; background: #05070a; display: flex; justify-content: center; align-items: center; user-select: none; overflow: hidden; cursor: grab; }}
    ...
    #bg-img {{ position: absolute; top:0; left:0; width: 581px; height: 481px; opacity: 0.5; pointer-events: none; }}
    #svg-canvas {{ position: absolute; top:0; left:0; width: 581px; height: 481px; z-index: 10; cursor: crosshair; }}

    .sidebar {{ width: 380px; background: #1e293b; border-left: 1px solid #334155; display: flex; flex-direction: column; padding: 14px; gap: 12px; overflow-y: auto; }}
    .section-title {{ font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.05em; color: #94a3b8; font-weight: 700; margin-bottom: 4px; }}
    
    .magic-card {{ background: linear-gradient(135deg, #064e3b, #1e1b4b); padding: 14px; border-radius: 10px; border: 2px solid #10b981; display: flex; flex-direction: column; gap: 10px; box-shadow: 0 0 15px rgba(16, 185, 129, 0.3); }}
    
    .question-card {{ background: #0f1117; padding: 12px; border-radius: 10px; border: 1px solid #334155; display: flex; flex-direction: column; gap: 8px; }}
    .question-header {{ display: flex; justify-content: space-between; font-weight: 700; color: #38bdf8; font-size: 0.9rem; }}
    .question-prompt {{ font-size: 0.8rem; color: #f8fafc; font-weight: 600; line-height: 1.3; background: #1e293b; padding: 8px; border-radius: 6px; border-left: 3px solid #38bdf8; }}

    .decision-buttons {{ display: grid; grid-template-columns: 1fr 1fr; gap: 6px; }}

    .pad-container {{ background: #0c1829; border: 2px solid #ec4899; padding: 12px; border-radius: 10px; display: flex; flex-direction: column; gap: 8px; align-items: center; }}
    .pad-area {{ width: 150px; height: 150px; background: #090d16; border: 2px solid #334155; border-radius: 12px; position: relative; cursor: crosshair; touch-action: none; overflow: hidden; display: flex; justify-content: center; align-items: center; }}
    .pad-grid-x {{ position: absolute; width: 100%; height: 1px; background: #1e293b; top: 50%; }}
    .pad-grid-y {{ position: absolute; height: 100%; width: 1px; background: #1e293b; left: 50%; }}
    .pad-knob {{ width: 24px; height: 24px; background: #ec4899; border: 2px solid #ffffff; border-radius: 50%; position: absolute; transform: translate(-50%, -50%); top: 50%; left: 50%; box-shadow: 0 0 12px rgba(236, 72, 153, 0.9); cursor: pointer; }}
    ...
    .control-group {{ display: flex; flex-direction: column; gap: 8px; background: #0f1117; padding: 10px; border-radius: 8px; border: 1px solid #334155; }}
    label {{ font-size: 0.78rem; color: #cbd5e1; display: flex; justify-content: space-between; }}

    textarea {{ width: 100%; height: 95px; background: #090d16; color: #10b981; border: 1px solid #10b981; padding: 8px; font-family: monospace; font-size: 0.72rem; border-radius: 6px; resize: none; }}

    /* TRANSPARENT HOLLOW RING ANCHOR NODES */
    .anchor-node {{ fill: none; stroke: #38bdf8; stroke-width: 2px; r: 6; cursor: pointer; transition: all 0.1s; }}
    .anchor-node:hover {{ stroke: #f43f5e; stroke-width: 3px; r: 8; }}
    .anchor-node.active-a {{ fill: none; stroke: #38bdf8; stroke-width: 3px; r: 9; stroke-dasharray: 2 2; }}
    .anchor-node.active-b {{ fill: none; stroke: #f43f5e; stroke-width: 3px; r: 9; filter: drop-shadow(0 0 6px #f43f5e); }}
    
    .handle-node {{ fill: rgba(251, 191, 36, 0.2); stroke: #fbbf24; stroke-width: 1.5px; r: 5; cursor: move; }}
    .handle-node:hover {{ fill: rgba(251, 191, 36, 0.8); }}
    .handle-line {{ stroke: #fbbf24; stroke-width: 1.5px; stroke-dasharray: 3 3; opacity: 0.8; }}
    .draft-path {{ stroke: #38bdf8; stroke-width: 1.5px; transition: opacity 0.1s; }}
...
<body>
  ...
  <header>
    <h1>⚡ Éditeur Vectoriel - Galbe Élevé à 46 px</h1>
    <div class="toolbar">
      <button class="btn btn-magic" onclick="runOneClickAutoVectorization()">⚡ Vectoriser en 1-Clic (Auto)</button>
      <button class="btn btn-secondary" onclick="zoomIn()">🔍 Zoom + (<span id="zoom-level">100%</span>)</button>
      <button class="btn btn-secondary" onclick="resetZoom()">🎯 Recadrer</button>
  ...
  <div class="main">
    ...
    <div class="sidebar">
      ...
      <div class="magic-card">
        <div style="font-weight: 700; color: #10b981; font-size: 0.95rem; display: flex; justify-content: space-between; align-items: center;">
          <span>⚡ Vectorisation IA 1-Clic</span>
          <span style="font-size: 0.75rem; background: #10b981; color: #0f1117; padding: 2px 6px; border-radius: 4px;">GALBE 46 PX</span>
        </div>
        <div style="font-size: 0.78rem; color: #e2e8f0; line-height: 1.35;">
          Hauteur d'élévation du galbe réglée à <b>46 px</b> pour surélever et épouser l'arc exact de la tranchée !
        </div>
        <button class="btn btn-magic" style="width: 100%; text-align: center;" onclick="runOneClickAutoVectorization()">🚀 ÉLEVER LES COURBES À 46 PX</button>
      </div>

      <!-- Question / Segment Card -->
      <div class="question-card">
        <div class="question-header">
          <span id="segment-step-title">Segment 7 / 18</span>
      ...
      <!-- 2D Pad (Contrôle Angle Droite & Galbe Courbe) -->
      <div class="pad-container">
        <div style="font-size: 0.8rem; font-weight: 700; color: #ec4899; width: 100%; display: flex; justify-content: space-between;">
          <span id="pad-mode-title">🕹️ Pad 2D : Galbe du Segment</span>
          <span id="pad-val-display" style="color: #fbbf24;">Galbe: -46 px</span>
        </div>

        <!-- 2D Interactive Trackpad -->
      ...
      <!-- Réglage d'Opacité -->
      <div>
        <div class="section-title">🎚️ Contrôle des Opacités</div>
        <div class="control-group">
          <label style="color: #38bdf8; font-weight: 700;">✨ Opacité des Traits Vectoriels: <span id="vector-stroke-opacity-val">80%</span></label>
          <input type="range" min="0" max="100" value="80" oninput="updateVectorStrokeOpacity(this.value)">

          <label style="margin-top: 4px;">🖼️ Opacité du Modèle de Fond: <span id="opacity-val">50%</span></label>
          <input type="range" min="0" max="100" value="50" oninput="updateOpacity(this.value)">
        </div>
      </div>

      <div style="flex: 1; display: flex; flex-direction: column;">
        <div class="section-title" style="color: #10b981;">Code SVG Transparent Résultant</div>
  ...
  <script>
    let currentSegmentIndex = 6;
    let isDraggingNode = false;
    let dragTarget = null;
    let vectorStrokeOpacity = 0.8;

    // Zoom & Mouse Panning Variables
    ...
    let allSegments = [
      ...
      {{ pIdx: 0, nIdx: 7, label: "Tranchée Supérieure Diagonale", desc: "Courbe élevée à 46px" }},
      {{ pIdx: 0, nIdx: 0, label: "Bord Extérieur Pilier Droit", desc: "Bord vertical extérieur droit" }},

      {{ pIdx: 1, nIdx: 0, label: "Lame Principale (Pointe Droit à Gauche)", desc: "Grande courbe élevée à 46px" }},
      {{ pIdx: 1, nIdx: 1, label: "Segment 10/18 : Pointe Extrême Gauche", desc: "Arc de courbe élevé à 46px" }},
    ...
    let paths = [
      {{
        ...
        nodes: [
          ...
          {{ x: 449, y: 205, type: 'C', arch: -46, c1x: 232, c1y: 225, c2x: 332, c2y: 194 }} // ELEVATED TO 46 PX!
        ]
      }},
      {{
        name: "Forme Inférieure",
        nodes: [
          {{ x: 569, y: 201, type: 'C', arch: -46, c1x: 404, c1y: 308, c2x: 224, c2y: 373 }}, // ELEVATED TO 46 PX!
          {{ x: 23, y: 396, type: 'C', arch: -46, c1x: 42, c1y: 338, c2x: 88, c2y: 325 }}, // ELEVATED TO 46 PX!
    ...
    function runOneClickAutoVectorization() {{
      paths[0].nodes = [
        ...
        {{ x: 147, y: 86, type: 'L', angle: 32, arch: 0 }},
        {{ x: 146, y: 301, type: 'L', angle: 90, arch: 0 }},
        {{ x: 449, y: 205, type: 'C', arch: -46, c1x: 232, c1y: 225, c2x: 332, c2y: 194 }}
      ];
      paths[1].nodes = [
        {{ x: 569, y: 201, type: 'C', arch: -46, c1x: 404, c1y: 308, c2x: 224, c2y: 373 }},
      ...
      render();
      alert('⚡ Élévation des courbes à 46 px effectuée ! La hauteur du galbe correspond désormais exactement au modèle.');
    }}
    ...
    function handlePadMove(e) {{
      ...
      if (node.type === 'L') {{
        ...
        node.angle = angleDeg;
        document.getElementById('pad-val-display').innerText = `Angle: ${{angleDeg}}°`;
        ...
        document.getElementById('pad-val-display').innerText = `Galbe: ${{archOffset}} px`;
    ...
    function updateUI() {{
      ...
      if (node.type === 'L') {{
        padTitle.innerText = "🕹️ Pad 2D : Angle de la Droite";
        presetBox.style.display = 'grid';
        document.getElementById('pad-val-display').innerText = `Angle: ${{node.angle || 90}}°`;
      }} else {{
        padTitle.innerText = "🕹️ Pad 2D : Galbe de la Courbe";
        presetBox.style.display = 'none';
        document.getElementById('pad-val-display').innerText = `Galbe: ${{node.arch || -46}} px`;
      }}
    }}
    ...
    function downloadSVG() {{
      ...
      a.download = 'logo_courbes_46px.svg';
      a.click();
    }}
...
print(f"Elevated 46px curve vector editor saved to {output_path}")
