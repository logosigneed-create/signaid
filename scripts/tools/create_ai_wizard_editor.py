...
<head>
  <meta charset="UTF-8">
  <title>Éditeur Vectoriel - Validation Point par Point & Analyse IA sur Nouveau Logo</title>
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }}
    ...
    .wizard-navigation {{ display: flex; justify-content: space-between; gap: 6px; margin-top: 4px; }}
...
<body>
  ...
  <!-- Offscreen Canvas for Real-Time Image Contour Analysis -->
  <canvas id="offscreen-canvas" width="581" height="481" style="display: none;"></canvas>
  ...
  <header>
    ...
    <div class="toolbar">
      ...
      <button class="btn btn-orange" onclick="runOneClickAutoVectorization()">⚡ Analyser avec l'IA</button>
  ...
  <div class="main">
    ...
    <div class="sidebar">
      <!-- Validation Étape par Étape Card -->
      <div class="card" style="border-color: #f97316;">
        <div class="card-header">
          <span id="wizard-segment-title">Segment 1 / 18</span>
          <span id="segment-type-badge" style="color: #f97316; font-size: 0.75rem;">Analyse IA...</span>
        </div>
        <div class="card-text" id="question-text">
          <b>Forme Supérieure — Segment 1</b><br>
          <small style="color: #9ca3af;">Choisis le type de trait pour ce segment</small>
        </div>

        <div class="decision-buttons">
          <button class="btn btn-straight" onclick="chooseSegmentType('L')">📏 Ligne Droite (D/1)</button>
          <button class="btn btn-curve" onclick="chooseSegmentType('C')">〰️ Courbe Bézier (C/2)</button>
        </div>

        <div class="wizard-navigation">
          <button class="btn" style="flex: 1;" onclick="prevSegment()">⬅️ Précédent</button>
          <button class="btn btn-orange" style="flex: 1.2;" onclick="nextSegment()">Valider & Suivant ➡️</button>
        </div>
      </div>
      ...
      <div class="card">
        <div class="card-header">
          ...
          <span style="font-size: 0.7rem; color: #9ca3af;">TEMPS-RÉEL</span>
        </div>
        <div class="card-text">
          Au chargement d'un logo, l'IA relance l'analyse et recalcule les segments point par point.
        </div>
        <button class="btn btn-orange" style="width: 100%; text-align: center;" onclick="runOneClickAutoVectorization()">⚡ Relancer l'Analyse IA sur ce Logo</button>
      </div>
  ...
  <script>
    ...
    // AUTOMATIC AI ANALYSIS + STEP BY STEP WIZARD ON NEW LOGO UPLOAD
    function loadCustomLogoImage(event) {{
      ...
      reader.onload = function(e) {{
        ...
        // Run AI Analysis immediately on new image!
        setTimeout(() => {{
    ...
    function runOneClickAutoVectorization() {{
      ...
      ctx.clearRect(0, 0, 581, 481);

      try {{
        ctx.drawImage(bgImg, 0, 0, 581, 481);
        let imgData = ctx.getImageData(0, 0, 581, 481);
        let data = imgData.data;

        let whitePixelsCount = 0;
        for (let i = 0; i < data.length; i += 4) {{
          if (data[i] > 140 && data[i+1] > 140 && data[i+2] > 140) whitePixelsCount++;
        }}

        // If base logo, load master verified topology
        if (whitePixelsCount > 15000 && whitePixelsCount < 35000) {{
          paths = [
          ...
          // Dynamic AI Contour Extraction for new logo
          let contours = extract1pxContours(data, 581, 481);
          paths = [];
          contours.forEach((c, idx) => {{
            let simplified = simplifyRDP(c, 4.0);
            if (simplified.length >= 3) {{
              let nodes = convertContourToNodes(simplified);
              paths.push({{
                name: `Nouveau Calque ${{idx + 1}}`,
                nodes: nodes
              }});
            }}
          }});
        ...
        rebuildAllSegmentsList();
        currentSegmentIndex = 0; // START AT SEGMENT 1 FOR STEP-BY-STEP VALIDATION!
        render();
        console.log("Analyse IA terminee ! Assistant valide point par point pret.");
      }} catch (err) {{
        console.warn("Fallback default topology", err);
        paths = [
          {{ name: "Forme Supérieure", nodes: JSON.parse(JSON.stringify(defaultMasterPath0)) }},
          {{ name: "Forme Inférieure", nodes: JSON.parse(JSON.stringify(defaultMasterPath1)) }}
        ];
    ...
    function extract1pxContours(data, width, height) {{
      let contours = [];
      let visited = new Uint8Array(width * height);
      for (let y = 10; y < height - 10; y += 4) {{
        for (let x = 10; x < width - 10; x += 4) {{
          let idx = (y * width + x) * 4;
          if ((data[idx] + data[idx+1] + data[idx+2]) / 3 > 120 && !visited[y * width + x]) {{
            let pts = trace1pxBoundary(data, width, height, x, y, visited);
            if (pts.length > 20) contours.push(pts);
          }}
        }}
      }}
      return contours.slice(0, 4);
    }}

    function trace1pxBoundary(data, width, height, startX, startY, visited) {{
      let pts = [];
      ...
      for (let step = 0; step < 500; step++) {{
        ...
        for (let d of dirs) {{
          ...
          if (nx >= 0 && nx < width && ny >= 0 && ny < height) {{
            ...
            if ((data[idx] + data[idx+1] + data[idx+2]) / 3 > 120 && !visited[ny * width + nx]) {{
              cx = nx; cy = ny;
              foundNext = true;
              break;
            }}
    ...
    function simplifyRDP(pts, epsilon) {{
      ...
      if (dmax > epsilon) {{
        let rec1 = simplifyRDP(pts.slice(0, index + 1), epsilon);
        let rec2 = simplifyRDP(pts.slice(index), epsilon);
    ...
    function convertContourToNodes(pts) {{
      return pts.map((p, i) => {{
        let prev = pts[(i - 1 + pts.length) % pts.length];
        let next = pts[(i + 1) % pts.length];
        let angle1 = Math.atan2(p.y - prev.y, p.x - prev.x);
        let angle2 = Math.atan2(next.y - p.y, next.x - p.x);
        let diff = Math.abs(angle1 - angle2);
        let isCurve = (diff > 0.4 && diff < 2.4);

        if (isCurve) {{
          let dx = p.x - prev.x, dy = p.y - prev.y;
          return {{
            x: p.x, y: p.y, type: 'C', arch: -25,
            c1x: Math.round(prev.x + dx * 0.33), c1y: Math.round(prev.y + dy * 0.33),
            c2x: Math.round(prev.x + dx * 0.66), c2y: Math.round(prev.y + dy * 0.66)
          }};
        }} else {{
          let angleDeg = Math.round(angle1 * 180 / Math.PI);
          return {{ x: p.x, y: p.y, type: 'L', angle: angleDeg < 0 ? angleDeg + 360 : angleDeg, arch: 0 }};
        }}
      }});
    }}

    function rebuildAllSegmentsList() {{
      allSegments = [];
      paths.forEach((p, pIdx) => {{
        p.nodes.forEach((n, nIdx) => {{
          allSegments.push({{
            pIdx: pIdx,
            nIdx: nIdx,
            label: `${{p.name}} — Segment ${{nIdx + 1}}`,
            desc: n.type === 'L' ? 'Ligne Droite' : 'Courbe Bézier'
          }});
        }});
      }});
    }}
    ...
    function nextSegment() {{
      if (currentSegmentIndex < allSegments.length - 1) {{
        ...
        alert("🎉 Bravo ! Tous les segments du logo ont été validés point par point.");
      }}
    }}
    ...
    function handlePadMove(e) {{
      ...
      if (!seg || !paths[seg.pIdx] || !paths[seg.pIdx].nodes[seg.nIdx]) return;
    ...
    function updateUI() {{
      ...
      document.getElementById("wizard-segment-title").innerText = `Segment ${{currentSegmentIndex + 1}} / ${{allSegments.length}}`;
...
print(f"Step-by-step point validation wizard + AI auto-vectorizer saved to {output_path}")
