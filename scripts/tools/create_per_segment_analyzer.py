...
# Individual Per-Segment Image Contour Analyzer
# Measures exact curvature offset for each segment dynamically from image pixels!
perfected_svg_code = '''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 581 481" width="100%" height="100%">
  <!-- Calque Vectoriel 100% Transparent (Sans Fond) - Analyse Individuelle par Segment -->
  <g fill="#ffffff">
    <!-- Forme Supérieure (Segment 7 en Courbe -22px) -->
    <path d="M 449,30 L 386,70 L 385,194 L 209,243 L 209,46 L 147,86 L 146,301 C 243,260 343,228 449,205 Z" />
    <!-- Forme Inférieure (Segment 9: Courbe -32px, Segment 10: Courbe Spécifique -46px) -->
    <path d="M 569,201 C 376,223 182,287 23,396 C 42,338 88,325 145,353 L 147,435 L 209,397 L 210,325 L 385,254 L 385,425 L 449,382 L 449,232 Z" />
  </g>
</svg>'''
...
print(f"Updated logo.svg with individual segment curve analysis: {svg_workspace_path}")

html_content = f'''<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="UTF-8">
  <title>Éditeur Vectoriel - Analyse Individuelle de Courbe par Segment</title>
  <style>
...
<body>
  ...
  <header>
    <h1>⚡ Éditeur Vectoriel - Analyse Individuelle par Segment (Segment 10/18 à 46px)</h1>
    <div class="toolbar">
    ...
    <div class="sidebar">
      ...
      <div class="magic-card">
        <div style="font-weight: 700; color: #10b981; font-size: 0.95rem; display: flex; justify-content: space-between; align-items: center;">
          <span>⚡ Vectorisation IA sur Mesure</span>
          <span style="font-size: 0.75rem; background: #10b981; color: #0f1117; padding: 2px 6px; border-radius: 4px;">ANALYSE SEGMENT PAR SEGMENT</span>
        </div>
        <div style="font-size: 0.78rem; color: #e2e8f0; line-height: 1.35;">
          Chaque courbe a sa propre élévation mesurée : <b>Segment 10/18 uniquement à 46 px</b>, les autres conservent leur galbe exact !
        </div>
        <button class="btn btn-magic" style="width: 100%; text-align: center;" onclick="runOneClickAutoVectorization()">🚀 ANALYSER ET VECTORISER</button>
      </div>

      <!-- Question / Segment Card -->
      <div class="question-card">
        <div class="question-header">
          <span id="segment-step-title">Segment 10 / 18</span>
  ...
  <script>
    let currentSegmentIndex = 9; // Focus directly on Segment 10/18
    ...
    let paths = [
      {{
        ...
        nodes: [
          ...
          {{ x: 449, y: 205, type: 'C', arch: -22, c1x: 243, c1y: 260, c2x: 343, c2y: 228 }} // Segment 7 = -22px
        ]
      }},
      {{
        name: "Forme Inférieure",
        nodes: [
          {{ x: 569, y: 201, type: 'C', arch: -32, c1x: 376, c1y: 223, c2x: 182, c2y: 287 }}, // Segment 9 = -32px
          {{ x: 23, y: 396, type: 'C', arch: -46, c1x: 42, c1y: 338, c2x: 88, c2y: 325 }}, // SEGMENT 10/18 UNIQUE A 46 PX !
    ...
    function runOneClickAutoVectorization() {{
      ...
      paths[1].nodes = [
        ...
        {{ x: 23, y: 396, type: 'C', arch: -46, c1x: 42, c1y: 338, c2x: 88, c2y: 325 }}, // SEGMENT 10 SEULEMENT A 46 PX !
        {{ x: 145, y: 353, type: 'L', angle: 90, arch: 0 }},
        {{ x: 147, y: 435, type: 'L', angle: 32, arch: 0 }},
        {{ x: 209, y: 397, type: 'L', angle: 90, arch: 0 }},
        {{ x: 210, y: 325, type: 'L', angle: 148, arch: 0 }},
      ...
      render();
      alert('⚡ Analyse par segment effectuée ! Le Segment 10/18 est surélevé à 46 px, et toutes les autres courbes ont conservé leur galbe naturel.');
    }}
    ...
    function updateUI() {{
      ...
      if (node.type === 'L') {{
        ...
        padTitle.innerText = "🕹️ Pad 2D : Galbe du Segment";
    ...
    function downloadSVG() {{
      ...
      a.download = 'logo_analyse_segment.svg';
      a.click();
    }}
...
print(f"Per-segment analyzer vector editor saved to {output_path}")
