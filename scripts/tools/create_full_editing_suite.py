...
<head>
  <meta charset="UTF-8">
  <title>Éditeur Vectoriel - Suite d'Édition Complète</title>
  <style>
...
<body>
  ...
  <header>
    <h1>✏️ Éditeur Vectoriel - Traçage & Édition Complète des Points</h1>
    <div class="toolbar">
      <button class="btn btn-secondary" onclick="prevStep()">⬅️ Préc. (Étape)</button>
      <button class="btn btn-success" onclick="validateStep()">✅ Valider Étape (Entrée)</button>
      <button class="btn btn-secondary" onclick="nextStep()">Suiv. ➡️</button>
    </div>
  </header>
  ...
  <div class="main">
    ...
    <div class="sidebar">
      <div>
        <div class="section-title">Outils d'Édition du Point Sélectionné</div>
        <div class="tool-grid">
          <button id="btn-mode-select" class="btn btn-active" onclick="setMode('select')">🖐️ Sélection / Déplacer</button>
          <button id="btn-mode-add" class="btn btn-secondary" onclick="setMode('add')">➕ Ajouter un Point</button>
          <button id="btn-mode-delete" class="btn btn-secondary" onclick="deleteSelectedNode()">🗑️ Supprimer Point</button>
          <button id="btn-mode-clear" class="btn btn-secondary" onclick="clearSelectedPath()">💥 Vider la Forme</button>
        </div>
      </div>

      <div>
        <div class="section-title">Convertir le Segment du Point</div>
        <div class="tool-grid">
          <button id="btn-convert-L" class="btn btn-secondary" onclick="convertSelectedSegment('L')">📏 Ligne Droite (L)</button>
          <button id="btn-convert-C" class="btn btn-secondary" onclick="convertSelectedSegment('C')">〰️ Courbe Bézier (C)</button>
        </div>
      </div>
  ...
  <script>
    ...
    let allSteps = [
      {{ pIdx: 0, nIdx: 0, label: "Point 1: Sommet Haut Droit" }},
      {{ pIdx: 0, nIdx: 1, label: "Point 2: Sommet Haut Gauche" }},
      {{ pIdx: 0, nIdx: 2, label: "Point 3: Entaille Haut Gauche" }},
      {{ pIdx: 0, nIdx: 3, label: "Point 4: Entaille Haut Droite" }},
      {{ pIdx: 0, nIdx: 4, label: "Point 5: Sommet Pilier Gauche" }},
      {{ pIdx: 0, nIdx: 5, label: "Point 6: Biseau Haut Gauche" }},
      {{ pIdx: 0, nIdx: 6, label: "Point 7: Bord Transversal Haut" }},
      {{ pIdx: 0, nIdx: 7, label: "Point 8: Entaille Transversale" }},
      
      {{ pIdx: 1, nIdx: 0, label: "Point 9: Pointe Droite Lame" }},
      {{ pIdx: 1, nIdx: 1, label: "Point 10: Courbe Lame Principale" }},
      {{ pIdx: 1, nIdx: 2, label: "Point 11: Pointe Gauche Lame" }},
      {{ pIdx: 1, nIdx: 3, label: "Point 12: Sommet Pilier Bas Gauche" }},
      {{ pIdx: 1, nIdx: 4, label: "Point 13: Bas Pilier Gauche" }},
      {{ pIdx: 1, nIdx: 5, label: "Point 14: Intérieur Pilier Gauche" }},
      {{ pIdx: 1, nIdx: 6, label: "Point 15: Entaille Basse" }},
      {{ pIdx: 1, nIdx: 7, label: "Point 16: Intérieur Pilier Droit" }},
      {{ pIdx: 1, nIdx: 8, label: "Point 17: Bas Pilier Droit" }},
      {{ pIdx: 1, nIdx: 9, label: "Point 18: Haut Pilier Droit" }}
    ];
    ...
    function updateUI() {{
      if (selectedNodeRef && paths[selectedNodeRef.pIdx] && paths[selectedNodeRef.pIdx].nodes[selectedNodeRef.nIdx]) {{
        let node = paths[selectedNodeRef.pIdx].nodes[selectedNodeRef.nIdx];
        document.getElementById('btn-convert-L').className = node.type === 'L' ? 'btn btn-active' : 'btn btn-secondary';
        document.getElementById('btn-convert-C').className = node.type === 'C' ? 'btn btn-active' : 'btn btn-secondary';
      }}
    }}
    ...
    svgCanvas.onclick = (e) => {{
      if (mode === 'add') {{
        ...
        let pIdx = selectedNodeRef ? selectedNodeRef.pIdx : 0;
        if (paths.length === 0) paths.push({{ name: "Forme", nodes: [] }});
        paths[pIdx].nodes.push({{ x: pos.x, y: pos.y, type: 'L' }});
        selectedNodeRef = {{ pIdx, nIdx: paths[pIdx].nodes.length - 1 }};
        render();
      }}
    }};
    ...
    function validateStep() {{
      if (currentStepIndex < allSteps.length - 1) {{
        currentStepIndex++;
        selectedNodeRef = {{ pIdx: allSteps[currentStepIndex].pIdx, nIdx: allSteps[currentStepIndex].nIdx }};
      }}
      render();
    }}
...
print(f"Full editing suite editor saved to {output_path}")
