...
<head>
  <meta charset="UTF-8">
  <title>Éditeur Vectoriel - Pad 2D pour Angle & Galbe</title>
  <style>
...
<body>
  ...
  <header>
    <h1>🕹️ Éditeur Vectoriel - Pad 2D Exclusif pour Angle & Galbe</h1>
    <div class="toolbar">
    ...
    <div class="sidebar">
      ...
      <div class="pad-container">
        <div style="font-size: 0.8rem; font-weight: 700; color: #ec4899; width: 100%; display: flex; justify-content: space-between;">
          <span id="pad-mode-title">🕹️ Pad 2D de Contrôle du Galbe</span>
  ...
  <script>
    ...
    // 2D Trackpad Pad (Exclusively Controls Angle & Arch Curve - Point Position is UNTOUCHED!)
    ...
    function handlePadMove(e) {{
      ...
      if (node.type === 'L') {{
        // Straight Line Mode: X-Axis of Pad controls Angle (0° to 360°)
        ...
        // Rotate line orientation without moving point A!
        node.x = Math.round(prev.x + len * Math.cos(rad));
        node.y = Math.round(prev.y + len * Math.sin(rad));
      }} else if (node.type === 'C') {{
        // Bezier Curve Mode: Pad X-Axis controls Arch Offset (-60 to +60 px)
    ...
    function downloadSVG() {{
      ...
      a.download = 'logo_pad2d_angle_galbe.svg';
      a.click();
    }}
...
print(f"Pad 2D Angle & Arch editor saved to {output_path}")
