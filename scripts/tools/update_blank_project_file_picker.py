...
<head>
  <meta charset="UTF-8">
  <title>Éditeur Vectoriel - Projet Vierge & Sélection de Logo</title>
  <style>
...
<body>
    ...
    <div class="sidebar">
      ...
      <div class="card" style="border-color: #f97316;">
        <div class="card-header">
          ...
          <span style="font-size: 0.7rem; color: #f97316;">PROJET VIERGE</span>
        </div>
        <div class="card-text">
          Sélectionnez une nouvelle image pour votre projet. L'arrière-plan précédent sera complètement effacé.
        </div>
        <button class="btn btn-upload" style="width: 100%; text-align: center;" onclick="triggerFileUpload()">📁 Choisir une Nouvelle Image</button>
      </div>
  ...
  <script>
    ...
    // NEW BLANK PROJECT: Clears background image and opens file picker for a brand new logo!
    function resetWorkspaceState() {{
      // 1. Clear background template image
      document.getElementById('bg-img').src = '';

      // 2. Clear vector paths and reset state
      paths = [];
      allSegments = [];
      ...
      // 3. Open file picker automatically for selecting the new logo image
      triggerFileUpload();
    }}
    ...
    function loadCustomLogoImage(event) {{
      ...
      reader.onload = function(e) {{
        ...
        // 3. Initialize fresh vector nodes for the newly uploaded logo
        paths = [
          {{
            name: "Nouveau Contour Logo",
            nodes: [
              {{ x: 120, y: 120, type: 'L', angle: 90, arch: 0 }},
              {{ x: 460, y: 120, type: 'L', angle: 0, arch: 0 }},
              {{ x: 460, y: 360, type: 'C', arch: -35, c1x: 340, c1y: 400, c2x: 220, c2y: 400 }},
              {{ x: 120, y: 360, type: 'L', angle: 90, arch: 0 }}
            ]
          }}
        ];
        ...
        render();
        alert('✨ Nouveau projet prêt ! L\'image de fond précédente a été retirée et le nouveau logo est chargé.');
      }};
...
print(f"Blank project file picker behavior updated in {output_path}")
