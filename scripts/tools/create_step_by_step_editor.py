...
<head>
  <meta charset="UTF-8">
  <title>Éditeur Vectoriel - Validation Étape par Étape Point par Point</title>
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Inter, sans-serif; }}
    ...
    .sidebar {{ width: 360px; background: #1e293b; border-left: 1px solid #334155; display: flex; flex-direction: column; padding: 16px; gap: 16px; }}
    ...
    .step-actions {{ display: flex; gap: 8px; margin-top: 4px; }}
    .step-actions .btn {{ flex: 1; text-align: center; justify-content: center; }}
    ...
    textarea {{ width: 100%; height: 140px; background: #090d16; color: #38bdf8; border: 1px solid #334155; padding: 10px; font-family: monospace; font-size: 0.75rem; border-radius: 6px; resize: none; }}
    ...
    .correction-stroke {{ fill: none; stroke: #f43f5e; stroke-width: 3px; stroke-linecap: round; }}
  </style>
</head>
<body>

  <header>
    <h1>🎯 Assistant de Validation Étape par Étape (Point par Point)</h1>
    <div class="toolbar">
    ...
    <div class="sidebar">
      ...
      <div class="step-card">
        ...
        <div class="step-desc" id="step-desc">
          Ajustez la position du point clignotant rouge en le faisant glisser ou en traçant par-dessus, puis validez.
        </div>
        <div class="step-actions">
          <button class="btn btn-success" onclick="validateCurrentStep()">✅ Valider Point & Suivant</button>
        </div>
      </div>
  ...
  <script>
    ...
    // All 17 Key Anchor Nodes with Descriptions
    let allSteps = [
      {{ pIdx: 0, nIdx: 0, label: "Point 1: Sommet Supérieur Droit (Pilier Droit)", desc: "Ajustez l'angle du biseau supérieur droit." }},
      {{ pIdx: 0, nIdx: 1, label: "Point 2: Sommet Supérieur Gauche (Pilier Droit)", desc: "Ajustez le coin gauche du biseau droit." }},
      {{ pIdx: 0, nIdx: 2, label: "Point 3: Bord Intérieur Inférieur (Pilier Droit)", desc: "Positionnez le départ de l'entaille supérieure." }},
      {{ pIdx: 0, nIdx: 3, label: "Point 4: Courbe de l'Entaille Supérieure", desc: "Ajustez la courbe ou le point d'arrivée de l'entaille." }},
      {{ pIdx: 0, nIdx: 4, label: "Point 5: Sommet Supérieur Droit (Pilier Gauche)", desc: "Ajustez le coin haut du pilier gauche." }},
      {{ pIdx: 0, nIdx: 5, label: "Point 6: Sommet Supérieur Gauche (Pilier Gauche)", desc: "Ajustez le biseau haut gauche." }},
      {{ pIdx: 0, nIdx: 6, label: "Point 7: Bord Extérieur Inférieur (Pilier Gauche)", desc: "Positionnez la limite de la tranchée gauche." }},
      {{ pIdx: 0, nIdx: 7, label: "Point 8: Courbe Extérieure Supérieure", desc: "Ajustez la grande courbe transversale supérieure." }},
      
      {{ pIdx: 1, nIdx: 0, label: "Point 9: Pointe Extrême Droite de la Lame", desc: "Positionnez la pointe de la lame acérée à droite." }},
      {{ pIdx: 1, nIdx: 1, label: "Point 10: Grande Lame Traversante", desc: "Ajustez la trajectoire de la lame vers la gauche." }},
      {{ pIdx: 1, nIdx: 2, label: "Point 11: Pointe Extrême Gauche de la Lame", desc: "Positionnez la pointe acérée en bas à gauche." }},
      {{ pIdx: 1, nIdx: 3, label: "Point 12: Sommet Extérieur Pilier Bas Gauche", desc: "Ajustez la jonction pilier/lame à gauche." }},
      {{ pIdx: 1, nIdx: 4, label: "Point 13: Bas du Pilier Gauche", desc: "Ajustez le biseau bas du pilier gauche." }},
      {{ pIdx: 1, nIdx: 5, label: "Point 14: Coin Intérieur Pilier Bas Gauche", desc: "Positionnez le coin intérieur gauche." }},
      {{ pIdx: 1, nIdx: 6, label: "Point 15: Entaille Inférieure Transversale", desc: "Ajustez la courbe de l'entaille basse." }},
      {{ pIdx: 1, nIdx: 7, label: "Point 16: Coin Intérieur Pilier Bas Droit", desc: "Positionnez le coin intérieur droit." }},
      {{ pIdx: 1, nIdx: 8, label: "Point 17: Bas du Pilier Droit", desc: "Ajustez le biseau bas du pilier droit." }}
    ];
    ...
    function render() {{
      ...
      // Draw active stroke if user is drawing over canvas
      if (userStroke.length > 1) {{
        let polyLine = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        let dStr = `M ${{userStroke[0].x}},${{userStroke[0].y}}`;
        for (let i = 1; i < userStroke.length; i++) {{
          dStr += ` L ${{userStroke[i].x}},${{userStroke[i].y}}`;
        }}
    ...
    window.onmouseup = () => {{
      if (isDrawingStroke && userStroke.length > 2) {{
        let endPt = userStroke[userStroke.length - 1];
        let step = allSteps[currentStepIndex];
        let node = paths[step.pIdx].nodes[step.nIdx];
        node.x = endPt.x; node.y = endPt.y;
      }}
    ...
    function validateCurrentStep() {{
      ...
      if (currentStepIndex < allSteps.length - 1) {{
        ...
        alert("🎉 Bravo ! Tous les 17 points ont été validés avec succès !");
      }}
      render();
    }}
...
print(f"Step-by-step vector validator saved to {output_path}")
