...
async function main() {
  console.log("=================================================");
  console.log("SYNCHRONISATION LOGO BLANC ET MOCKUPS TEXTILES FIRESTORE");
  console.log("=================================================\n");

  const primaryApp = initializeApp(firebaseConfigs[0], firebaseConfigs[0].name);
  const storage = getStorage(primaryApp);

  const logoLocal = path.resolve(__dirname, 'public/logo_dfazz_white.png');
  const tshirtLocal = path.resolve(__dirname, 'public/tshirt_dfazz_white_mockup.png');
  const poloLocal = path.resolve(__dirname, 'public/polo_dfazz_white_mockup.png');
  const hoodieLocal = path.resolve(__dirname, 'public/hoodie_dfazz_white_mockup.png');

  const ts = Date.now();
  console.log("1. Upload du logo blanc officiel vers shared_media/btp_mockups/audit-8f198p5/...");
  ...
  console.log("2. Upload des rendus textiles haute définition vers /web/...");
  ...
  // Sanitize payload to guarantee no base64 string remains
  ...
  for (const cfg of firebaseConfigs) {
    ...
    let app;
    try {
      app = initializeApp(cfg, `${cfg.name}_db_app_v5`);
    } catch (e) {
      app = primaryApp;
    }
    ...
    // Affichage des JSON exacts sanitizés
    ...
    console.log(`\n📄 JSON SANITIZÉ FIRESTORE [SiteConfigs/guest_ms3ijgnco2xnid] (${cfg.name}) :`);
...
main().catch((err) => {
  console.error("❌ Erreur fatale dans syncAuditDataWithSanitize.cjs :", err);
  process.exit(1);
});
