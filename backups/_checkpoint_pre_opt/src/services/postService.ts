...
class PostService {
    ...
    async getPosts(lastDoc: any = null, limitCount: number = 100): Promise<{ posts: Post[], lastDoc: any }> {
        try {
            // FIX: Removed server-side orderBy('createdAt') to prevent Firestore 400 Bad Request (Missing Index).
            // We fetch posts and sort them client-side.
            let q;
            if (lastDoc) {
                // Note: startAfter with unordered query uses document reference/ID, which works.
                q = query(this.postsCollection, startAfter(lastDoc), limit(limitCount));
            } else {
                q = query(this.postsCollection, limit(limitCount));
            }
            ...
            console.log(`[PostService] Fetched ${posts.length} posts (Client-Side Sorted).`);
