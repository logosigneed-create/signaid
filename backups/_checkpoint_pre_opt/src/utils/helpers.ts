...
export async function trimImage(imageSrc: string, tolerance: number = 20): Promise<{ 
    ...
    try {
        ...
        const ctx = canvas.getContext('2d');
        if (!ctx) return { dataUrl: imageSrc, widthRatio: 1, heightRatio: 1, cropTop: 0, cropLeft: 0, originalWidth, originalHeight };
        ...
        if (!found) {
            return { dataUrl: imageSrc, widthRatio: 1, heightRatio: 1, cropTop: 0, cropLeft: 0, originalWidth, originalHeight };
        }
        ...
        if (!cropCtx) return { dataUrl: imageSrc, widthRatio: 1, heightRatio: 1, cropTop: 0, cropLeft: 0, originalWidth, originalHeight };
        ...
        // Fallback with dummy values to match type
        return { dataUrl: imageSrc, widthRatio: 1, heightRatio: 1, cropTop: 0, cropLeft: 0, originalWidth: 2000, originalHeight: 2000 };
    }
}
