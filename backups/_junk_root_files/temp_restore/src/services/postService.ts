...
class PostService {
    ...
    private async processObjectForUpload(obj: any): Promise<any> {
        ...
        if (typeof obj === 'object') {
            ...
            for (const key in obj) {
                newObj[key] = await this.processObjectForUpload(obj[key]);
            }
            return newObj;
        }
        return obj;
    ...
    async getPosts(lastDoc: any = null, limitCount: number = 100): Promise<{ posts: Post[], lastDoc: any }> {
        try {
            // SAFE LIMIT + ORDER BY NEWEST
            // Try to get newest posts. If index is missing, catch error and fallback.
            try {
                let q;
                if (lastDoc) {
                ...
                const querySnapshot = await getDocs(q);
                const posts = this.processDocs(querySnapshot);
                const lastVisible = querySnapshot.docs[querySnapshot.docs.length - 1];
                return { posts, lastDoc: lastVisible };

            } catch (indexError) {
                console.warn("Index missing for orderBy(createdAt), falling back to unordered:", indexError);
                // Fallback: Unordered (random/insertion) but limited
                // Note: Pagination without specific order relies on document ID or undefined order, prone to issues but acceptable for fallback
                let qFallback;
                if (lastDoc) {
                    qFallback = query(this.postsCollection, startAfter(lastDoc), limit(limitCount));
                } else {
                    qFallback = query(this.postsCollection, limit(limitCount));
                }
